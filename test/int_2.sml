...
val maxint : int = 
  case precision 
    of SOME 32 => 2 * 1073741823 + 1
     | SOME 31 => 1073741823
     | NONE => raise Fail "maxint"
...
val test5 = test "test5" (case precision
			    of SOME 32 => ((if maxint = 2 * 0x3fffffff + 1 then "OK" else "WRONG")
					   handle Overflow => "EXN")
			     | SOME 31 => if maxint = 0x3fffffff then "OK" else "WRONG"
			     | _ => "WRONG")
...
val test7 = test "test7" (case precision
			    of SOME 32 => ((if minint = 2 * ~0x40000000 then "OK" else "WRONG")
					   handle Overflow => "EXN")
			     | SOME 31 => if minint = ~0x40000000 then "OK" else "WRONG"
			     | _ => "WRONG") 
...
val test45 = test "test45" ((fromLarge(Int32.toLarge(valOf Int32.maxInt)) seq (if tagging() then "WRONG" else "OK"))
