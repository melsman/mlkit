(* Test cases for Word<N>Vector, Word<N>VectorSlice, Word<N>Array, and
   Word<N>ArraySlice structures. PS 1994-12-10, 1995-06-14, 2000-10-24
...
struct
  ...
  fun pr_section s =
      print ("[Test begin: Word"
             ^ (if !pr_n then Int.toString W.wordSize else "")
             ^ s ^ "]\n")
  ...
  val test1a =
    check'(fn _ => List.all
	   (fn sli => eqv(vector sli, fromList [])
	    ...
	    andalso collate W.compare (sli, slice00) = EQUAL)
	   slice0s)
  ...
  fun setv c = v := c
  fun addv c = v := W.+(c, !v)
  fun setvi (i, c) = v := W.+(c, i2w i)
  fun setvif (i, c, _) = v := W.+(c, i2w i)
  fun addvi (i, c) = v := W.+(c, W.+(i2w i, !v))
  fun cons (x,r) = x :: r
  fun consi (i,x,r) = (i,x) :: r
  ...
  val test14b =
    check'(fn _ =>
	   List.all (fn sli => NONE=findi (fn (j, x) => Int.>=(j,7) orelse Int.<(j, 0) orelse W.mod(x, i2w 10) <> i2w 1) sli)
	            sliceas)
  ...
  let val sa = fromString "AAAAaAbAABBBB"
                                (* 0123456789012 *)
      fun invcompare (c1, c2) = W.compare (c2, c1)
  ...
  val test12 =
    check'(fn _ =>
	   let fun invcompare (c1, c2) = W.compare(c2, c1)
	       val fromString =
		   fromList o List.map (W.fromInt o ord) o explode
	       fun coll s1 s2 =
  ...
  val test1a =
    check'(fn _ => List.all
	   (fn sli => eqv(vector sli, l2v [])
	    ...
	    andalso foldr cons [i2w 1,i2w 2] sli = [i2w 1,i2w 2]
	    andalso foldri consi [] sli = []
	    andalso (modify W.~ sli; eqv(vector sli, l2v []))
	    andalso (modifyi (fn (_, x) => W.~ x) sli; eqv(vector sli, l2v []))
	    andalso collate W.compare (sli, slice00) = EQUAL)
	   slice0s)
...
val test12c =
  ...
  val test12e =
    check'(fn _ =>
	   (resetinp(); modify W.~ inp;
	    foldr (op::) [] inp = [W.~ (i2w 7), W.~ (i2w 9), W.~ (i2w 13)])
  ...
  val test14b =
    check'(fn _ =>
	      List.all (fn sli => NONE=findi (fn (j, x) => Int.>=(j, 7) orelse Int.<(j, 0)
                                                           orelse W.mod(x, i2w 10) <> i2w 1) sli)
	            sliceas)
  ...
  val test16 =
    check'(fn _ =>
	   let fun iwsub (i, w) = W.-(i2w i, w)
	   in
	       (resetinp(); modifyi iwsub inp;
		eqv(vector inp, V.fromList (List.map W.fromLargeInt [~7,~8,~11])))
	       andalso (resetinp(); setv (i2w 117);
