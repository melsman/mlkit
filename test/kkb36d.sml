...
local
  ...
  *)
  ...
  fun it_list2 f a [] [] = a
    | it_list2 f a (a1::L1) (a2::L2) = it_list2 f (f (a,a1,a2)) L1 L2
    | it_list2 _ _ _ _ = raise FailItList2
  
  fun exists p [] = false
    | exists p (a::L) = p a orelse exists p L
  ...
  fun try_find f [] = raise FailTryFind
    | try_find f (a::L) = (f a) handle _ => try_find f L
  
  fun partition p [] = ([],[])
    | partition p (a::L) =
      let val (pos,neg) = partition p L in
  ...
  *)
  ...
  fun union eq [] L2 = L2
    | union eq (a::L) L2 = if mem eq a L2 then union eq L L2 else a :: union eq L L2
  ...
  *)
  ...
  fun assoc eq a [] = raise FailFind
    | assoc eq a ((b,d)::L) = if eq(a,b) then d else assoc eq a L
  ...
  fun union eq L1 [] = L1
    | union eq L1 (a::L) = if mem eq a L1 then union eq L1 L else a :: union eq L1 L
  ...
  fun copy_term (VVar (n,_)) = Var n
    | copy_term (Term(s, l)) = Term(s, map copy_term l)
  
  fun eq_term x =
    (fn (VVar (i1,_), VVar (i2,_)) => 
         eq_integer(i1,i2)
  ...
  *)
  fun substitute subst (t as Term(oper,[])) = t
    ...
    | substitute subst (t as (VVar (n,_)))     = (assoc eq_integer n subst) handle _ => t
  
  fun change f (h::t) n = if eq_integer(n,1) then f h :: t
			  else h :: change f t (n-1)
    | change _ _ _ = raise FailChange
  ...
  fun matching term1 term2 =
    let fun match_rec (subst, VVar (v,_), M) =
  	  ...
  	  if eq_string(op1,op2) then it_list2 (fn (a,b,c) => match_rec (a,b,c)) subst sons1 sons2
                         else raise FailMatching
          | match_rec _ = raise FailMatching
  ...
  *)
  ...
  fun rem_eq equiv x [] = raise FailRemEQ
    | rem_eq equiv x (y::l) = if equiv (x,y) then l else y :: rem_eq equiv x l
  ...
  *)
  ...
  fun rename n (t1,t2) =
    let fun ren_rec (VVar (k,_)) = Var(k+n)
  ...
  fun copy_int_pair(x,y) = (x,y)
  fun copy_int_pair_list l = map copy_int_pair l
  
  fun copy_arg(interm:bool, n, rules, failures, k, l, eps) =
      (interm, n, copy_rules rules, copy_termpairlist failures, k, l, copy_termpairlist eps)
  
  (* Improved Knuth-Bendix completion procedure *)
  (* kb_completion :  num -> rules -> term_pair list -> (num & num) -> term_pair list -> rules *)
  fun kb_completion (arg as (done,n, rules, list, k, l, eps)) =
    let fun kbrec count n rules failures k l llll =
      let fun normal_form x = mrewrite_all rules x
          fun get_rule k = assoc eq_integer k rules
      in
              case llll of
		  [] =>
                if k<l then next_criticals count n rules failures (k+1) l else
                if l<n then next_criticals count n rules failures 1 (l+1) else
                 (case failures of
              ...
              | ((M,N)::eqs) =>
                ...
                in if eq_term(M',N') then kbrec count n rules failures k l eqs else
                   if greater(M',N') then enter_rule( M', N') 
                   else
                   if greater(N',M') then enter_rule( N', M') 
                   else
                         kbrec count n rules  ((M', N')::failures) k l eqs
                end
      end
            and next_criticals count n rules failures k l =
              (let fun get_rule k = assoc eq_integer k rules
		   val (v,el) = get_rule l in
                 if eq_integer(k,l) then
                   kbrec count n rules failures k l (strict_critical_pairs el (rename v el))
                 else
                  (let val (_,ek) = get_rule k in 
                      kbrec count n rules failures k l (mutual_critical_pairs el (rename v ek))
                   end
                   handle FailFind (*rule k deleted*) =>
                     next_criticals count n rules failures (k+1) l)
               end
               handle FailFind (*rule l deleted*) =>
                  next_criticals count n rules failures 1 (l+1))
  
      fun kb_outer (* [r2517] *)(arg as (_, n, rules, failures, k, l, other_failures)) =
        case kbrec 1 n rules failures k l other_failures of
          result as (true,_, result_rules,_,_,_,_) => if false then arg else result
    ...
    in (fn (_,_,x,_,_,_,_) => x)(kb_outer(arg))
    end
  ...
  fun kb_complete  complete_rules (* the terms in the complete_rules are global *) rules =
      let val n = check_rules complete_rules
          ...
          val completed_rules = 
                 ...
                 kb_completion(false,n, copy_rules complete_rules, [], n,n, copy_termpairlist eqs) 
