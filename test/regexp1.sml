(* Testing the RegExp module; ported from Moscow ML test of the POSIX
 * regex interface *)
...
open RegExp;
val match' = match o fromString
val extract' = extract o fromString
local

  val neg = [("ab+c", ["", "ab+c", "ab bbbbc", "abbcL", "Kabbc", " abbc", "abbc "]),
  ...
  val pos = [("ab+c", ["abc", "abbc", "abbbbbc"]),
	     ("ab+c|[de]*f", ["f", "ddef", "ddeedf"]),
	     ("ab+c|([de]*)f", ["f", "ddef", "ddeedf"]),
	     ("(æ|ø|å)", ["æ","ø","å"])];
...
val test1a = 
  tst' "test1a" (fn _ =>
		 let (* All these patterns raises Fail msg *)
		   val pats = ["", "(", "(()"] 
		 in
		   List.all (fn r => ((fromString r; false) handle Fail _ => true)) pats
		 end)

(* match *)
val test2a = 
  tst' "test2a" (fn _ => 
		 List.all (fn (pat, ss) => not (List.exists (match' pat) ss))
		 neg)

val test2b = 
  tst' "test2b" (fn _ => 
		 List.all (fn (pat, ss) => List.all (match' pat) ss)
		 pos)

(* extract *)
val test3a = 
   tst' "test3a" (fn _ => 
		 List.all (fn (pat, ss) => not (List.exists (Option.isSome o extract' pat) ss))
		 neg)

val test3b = 
  tst' "test3b" (fn _ => 
		 List.all (fn (pat, ss) => List.all (Option.isSome o extract' pat) ss)
		 pos)
