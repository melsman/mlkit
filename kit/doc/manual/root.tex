...
DENMARK\\
\ \\
November 1998}\vspace{4.5cm}

{\Large Programming with Regions in the ML Kit\\ (for Version~3)}\vspace{2cm}

{\large Mads Tofte, Lars Birkedal, Martin Elsman, Niels Hallenberg, Tommy H{\o}jfeld Olesen, Peter Sestoft and Peter
     Bertelsen }\vfill
...
de-allocation instructions (e.g., \index{malloc@\texttt{malloc}}{\tt
  malloc} and \index{free@\texttt{free}}{\tt free} in C). For non-trivial
...
\end{verbatim}
Here \boxml{nthgen'} \index{nthgen@\texttt{nthgen}}is the main loop of
...
passionately about it).\footnote{The source file for the life program
  is \boxml{kitdemo/life.sml}\index{life@\texttt{life}}. Running programs
...
If $e$ is an expression in $\RegExp$, then so is\index{letregion@\texttt{letregion}}
...
Running the target program is done from the UNIX shell by typing \index{run@\texttt{run}}
\begin{verbatim}
       run
\end{verbatim}
...
\index{at@\texttt{at}}where $\rho$ is a 
\index{region variable}%
...
when it is safe to put \boxml{letregion} around  an expression.\index{letregion@\texttt{letregion}}
...
where the \index{letregion@\texttt{letregion}}{\tt letregion} ends?\index{region!de-allocation}
...
algorithm wraps a \index{letregion@\texttt{letregion}}{\tt letregion} binding of the region variable
...
(the value of type \index{unit@\texttt{unit}}{\tt unit}) is represented
...
expression of the form \index{at@\texttt{at}}$c\at\rho$, where $\rho$ is
...
from the two constructors \index{::@\texttt{::}}\boxml{::} (read: cons)
and \index{nil@\texttt{nil}}{\tt nil}.   
...
by applying a \index{decon@\texttt{decon}}deconstructor primitive,
not by pattern matching.\index{at@\texttt{at}}
\begin{center}
...
\index{fun@\texttt{fun}}{\tt fun} and that are first-order (i.e., that neither take functions
...
do not use brackets for lists.\index{[ ]@\texttt{[ ]}}.
...
\label{fromto.fig}
\end{figure}
Notice that \boxml{r7} is a formal region parameter of {\tt fromto}.\index{fromto@\texttt{fromto}}
...
\index{cp@\texttt{cp}}\index{msort@\texttt{msort}}
\index{merge sort}%
...
Although the scope of the declaration is only the declaration of \index{smallPrime@\texttt{smallPrime}}{\tt smallPrime},
...
Basis Library:\index{TextIO@\texttt{TextIO}}\index{openIn@\texttt{openIn}}\index{openOut@\texttt{openOut}}
\begin{verbatim}
...
\index{ref@\texttt{ref}}\index{"!@\texttt{"!}}\index{:=@\texttt{:=}}
\medskip
...
\index{tree@\texttt{tree}}\index{Lf@\texttt{Lf}}\index{Br@\texttt{Br}}\index{datatype@\texttt{datatype}}
\begin{verbatim}
       datatype 'a tree = Lf | Br of 'a * 'a tree * 'a tree;
\end{verbatim}
...
consider an expression of the form \index{letregion@\texttt{letregion}}
...
Here is an example of prudent use of exceptions in the ML Kit:\index{hd@\texttt{hd}}\index{tl@\texttt{tl}}
\bigskip
...
\item[B1] {\it $\rho$ is bound outside the smallest enclosing lambda abstraction
      ...
      formal parameter):}
      use {\tt attop} (see Figure~\ref{b1.fig})\index{attop@\texttt{attop}};
\item[B2]{\it $\rho$ is bound by a {\tt letregion} expression inside the
      ...
      (see Figure~\ref{b2.fig})\index{letregion@\texttt{letregion}};
    \item[B3 (first attempt)]{\it $\rho$ is a formal parameter of a
        region-polymorphic function whose right-hand side is the
        smallest enclosing lambda abstraction:} use \index{sat@\texttt{sat}}{\tt sat}, if no locally live variable at the
...
declared outside {\tt llength}, using \index{local@\texttt{local}}{\tt
  local}.
\begin{figure}
\vbox{
\hrule
...
\caption{Region profiling of the improved mergesort. 
  ...
  profiling enabled and then run with the command \boxml{run -microsec 100000}. 
  The PostScript picture \boxml{region.ps} was generated
  with the command \boxml{rp2ps -region -sampleMax 1000 -eps 137 mm}
...
\index{scan@\texttt{scan}}scan a sequence of Standard ML source
...
Every occurrence of {\tt fn} \index{fn@\texttt{fn}}in the program is considered 
...
declared with \index{val@\texttt{val}}{\tt val} do not in general become
\index{region polymorphism}%
...
to functions. It is called \index{standardArg@\texttt{standardArg}}{\tt stardardArg}. In addition, region-polymorphic functions
use another fixed register, 
called \index{standardArg1@\texttt{standardArg1}}{\tt standardArg1}\footnote{Admittedly, not terribly good nomenclature.},
...
a function call as a simple \index{jump}\index{jmp@\texttt{jmp}}
\index{region polymorphism}%
...
\section{Non-Tail Call of Region-Polymorphic Function \index{funcall@\texttt{funcall}}({\tt funcall})}
...
register called \index{standardClos@\texttt{standardClos}}\index{register}
{\tt standardClos}. Then $\exp_2$ will be evaluated and the result put
in the standard register \index{standardArg@\texttt{standardArg}}{\tt standardArg}.
...
a non-tail call of \boxml{g} and then a tail call to \boxml{f}.\index{o@\texttt{o}}
...
\end{verbatim}
The recursive call of \index{foldl@\texttt{foldl}}{\tt foldl} is a call of a known function, but not a tail
...
linked, it can be executed by running the command\index{run@\texttt{run}}
\begin{verbatim}
 run
\end{verbatim}
...
{\tt Profiling} sub-menu:\index{instruction count profiling@\texttt{instruction count profiling}}\footnote{The   
...
\index{show region flow graph and generate .vcg file@\texttt{show region flow graph and generate .vcg file}}%
...
\begin{figure}
\begin{center}
  \scalebox{0.6}{\includegraphics{vcg3.ps}}
\end{center}
...
\begin{figure}
\begin{center}
  \scalebox{0.6}{\includegraphics{vcg4.ps}}
\end{center}
...
leave the Kit, type \index{quit@\texttt{quit}}\boxml{quit} followed by a
return character. 
...
\index{print atbot expression@\texttt{print atbot expression}}%
...
\index{print KAM code before register allocation@\texttt{print KAM code before register allocation}}%
\texttt{print KAM code before register allocation}. The result of
\index{register allocation}%
register allocation can be viewed by enabling the flag 
\index{print KAM code after register allocation@\texttt{print KAM code after register allocation}}%
...
\texttt{Layout}\index{Layout@\texttt{Layout}} control how these forms are
printed.
...
\index{kit.script@\texttt{kit.script}}%
...
The Kit contains a module, StatObject,\index{StatObject}
...
\nocite{total97,total94,btv96,elshal95,koch&olesen}
