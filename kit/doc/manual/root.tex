...
% {\LARGE Programming with Regions in the MLKit}\vspace{2mm}

% {\LARGE (for Version~4.1.1)}\vspace{1cm}
...
\index{MLKit!Version 4}%
MLKit Version~4. This version of the MLKit extends the 
...
\begin{verbatim}
   mlkit
\end{verbatim}
Select \boxml{Profiling} from the MLKit menu (type 4, then carriage
...
(including the quotes, then return). After the MLKit has compiled
...
The MLKit provides two mechanisms for compiling programs. The first
...
In both cases, you need an executable version of the MLKit; let us
...
MLB-files to Chapter~\ref{mlb_and_modules.chap}.
...
Example~\ref{proj.ex}, place yourself in your own copy of the {\tt
  kitdemo} directory and start the MLKit with the shell command {\tt
  mlkit}.

After the MLKit has uttered various greetings, you will find yourself in
...
>
\end{verbatim}
\caption{The top-most MLKit menu}
\medskip \hrule 
\label{dialogue.fig}
\end{figure}
First, you are going to ask the MLKit to print one of the intermediate
...
The MLKit outputs (among other things) the $\MulExp$ program shown in
Section~\ref{proj.ex}. 
...
compile the source file {\tt projection.sml} again. This time, the MLKit
...
As mentioned earlier, it is possible to compile programs with the MLKit
...
\end{verbatim}
To get more information about which options you can pass to the MLKit at
...
MLKit, because it will create one closure for each element in the list,
due to currying.\footnote{When {\tt map} and the application of {\tt
    map} appear in the same compilation unit, the MLKit will
...
detect it? If not, the MLKit will tell you when you compile the program.
...
large with the MLKit, using 
\index{Standard ML!Modules}% 
Standard ML Modules and the possibility of organising source files
into project files. The MLKit fully supports Standard ML Modules and it
...
separated by a punctuation letter (\texttt{.}).
...
\caption{Grammar for MLB-files, i.e., files with extension {\tt .mlb}. 
  For some file extension {\tt .}{\it ext}, 
  {\it path}{\tt .}{\it ext\/} denotes either an absolute path or a
  relative path (relative to the directory in which the project file
  is located) to a file on the underlying file system.}
\label{mlb_grammar.fig}
\medskip \hrule
\end{figure}
In an MLB-file, one can reference source files and other MLB-files using absolute or relative
\index{path!absolute}%
\index{path!relative}%
...
variable. Until now, we have seen a few examples of MLB-files that
reference the Basis Library, using the \texttt{\$(SML\_LIB)}
environment variable (see Section~\ref{polyrec.sec} for such an
example). In Section~\ref{functors.sec}, we present an example of an
MLB-file that reference other MLB-files. In
Section~\ref{comp_and_link_with_C.sec}, we shall see an example of how an MLB-file can be compiled and linked with external object files, produced with a C compiler, for instance. MLB-files may contain
Standard ML style
...
project, consult the MLB-file {\tt basis/basis.mlb}.
...
declaration that would arise by expanding all referenced MLB-files
and then concatenating all the source files listed in the MLB-file
...
\mbox{{\tt region profiling}}. If you want the MLKit to report
...
the main-menu, the MLKit now generates several files of which we have
...
% from one region variable to another, the MLKit can find them for you.
...
%   necessary to start all over by starting the MLKit again and recompile
...
%---------------------------------------------------------
\chapter{Interacting with the MLKit}
\label{controlkit.sec}
\label{startup.sec}
%---------------------------------------------------------

Starting the MLKit was described in Section~\ref{tryit.sec}.  To
\index{leaving the MLKit}%
\index{exit from the MLKit}%
\index{quitting from the MLKit}%
leave the MLKit, type 
...
we give an overview of the MLKit sub-menus that control printing and
...
for menu entries in the MLKit. The settings that can be controlled using
the menu and script files can also be controlled using command-line
options to the MLKit executables. One useful command-line option is
the 
...
\caption{The table shows how the menu items in the 
  ...
  in the MLKit.  Enabling \boxml{debug compiler} from the \boxml{Debug
    MLKit} menu causes all intermediate forms marked $(\ast)$ to be
...
The MLKit allows you to create a so-called 
\index{script file}%
{\em script file}, which can hold preferences for most of the entries
found in the MLKit menu. A script file can be provided either when the
MLKit is started or dynamically from within the MLKit menu when the MLKit is
running. To provide a script file {\tt script} at the time the MLKit is
started, type
\begin{verbatim}
   mlkit -script script
\end{verbatim}
from the shell. During a session with the MLKit, a script file can be
...
sub-menu, the MLKit prints a table of all entries that may be set from a
script file.
...
\section{Linking at Runtime}
\label{link_at_runtime.sec}
The MLKit support dynamic linking at runtime.
This is done using the \texttt{dlopen} and \texttt{dlsym} functions from the
kit library. The function \texttt{dlopen} opens a given library and
the function \texttt{dlsym} associates a name to a given function in the
library. If the name already has an association the exception `Fail' is
raised.

Using \texttt{dlopen} and \texttt{dlsym} as in figure~\ref{dynlib.fig},
you can call a dynamically linked library function by a primitive call
to `:'.
...
\end{verbatim}
\caption{Dynamic linking of the function ``FascistCheck'' from the library ``libcrack.so''.
  The ml-function fascistCheck calls FascistCheck with the argument
  (a,/usr/lib/cracklib\_ dict) and converts the resulting c-string into
  an ml-string.
...
This chapter provides an overview of the main changes to the MLKit
since version 3.0.
...
{\tt Control} menu or start the MLKit with the option {\tt -gc}. Garbage
...
\begin{verbatim}
MLKit version 4.1.1, Aug 25, 2002 [X86 Backend]

Usage: mlkit_kam [OPTION]... [file.sml | file.sig | file.pm]
...
--no_recompile_basislib, -no_scratch
     Opposite of --recompile_basislib, -scratch.
...
--strip                                                   (off)
     If enabled, the MLKit strips the generated executable.
