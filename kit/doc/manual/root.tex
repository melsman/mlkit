...
non-existing global region.\index{boxing}\index{value!boxed}\index{value!unboxed} As an
...
actual region parameters thus telling the function where to put its
...
For an example, consider\footnote{Program \boxml{kitdemo/fromto.sml}.}
\begin{verbatim}
...
\label{regtych.sec}
A 
\index{type scheme!region-polymorphic}%
...
\index{region type scheme and place}%
...
shorten \index{lifetime|)}\index{lifetime!shortening}lifetimes
include:
\begin{enumerate}
\item Inwards let floating: 
\index{let floating}%
transform
...
disabled:\footnote{This disabling is done by setting the {\tt
    maximum inline size}, found in the {\tt Control/Optimiser} sub-menu, to {\tt 0}.}
\begin{figure}
\hrule
\begin{verbatim}
...
was generated by the \index{sampleMax@{\tt sampleMax}}
\index{eps file}%
\index{rp2ps@{\tt rp2ps}}%
...
and {\tt f}.  Whereas functions declared with \index{fun@{\tt
    fun}}{\tt fun} automatically become region-polymorphic, functions
...
\index{type!region-annotated}function type is
$$(\tau_1\ar{\epsilon.\rea}\tau_2, \rho)$$
where $\tau_1$ is the
region-annotated type of the argument, $\tau_2$ is the
region-annotated type of the result, and $\rho$ is the region
...
have different latent effects. Let us take the function {\tt (fn n =>
  n + 1.0)} as example. It has region-annotated function type
\begin{equation}
\label{suc.lab}
...
arrow effects.  We shall not define here what it means to apply a
substitution to a type, but let us explain why substitutions map
...
\index{region type scheme!printing of}%
types of all binding occurrences of value variables.  Also, one can
choose to have arrow effects included in the printout: Enable the
flags~~\texttt{print types}~~and~~\texttt{print effects}~~in the
\texttt{Layout} menu.  Although enabling these flags gives very
verbose output, it is instructive to look at such a term just once, to
...
\label{map.fig}
\end{figure}
We see that the region that appear free in the region-annotated type of the successor
...
The solution is to require that {\tt f} be curried, to avoid the creation of the pair altogether, that is, going to higher order
rather than lower:\footnote{Program \boxml{kitdemo/fold2.sml}.}
\begin{verbatim}
       fun foldl f b xs = 
...
\end{verbatim}
The region-annotated version of this program appears in Figure~\ref{fold2.fig} on page~\pageref{fold2.fig}.
...
An efficient version of {\tt foldl} is written thus:\footnote{Program \boxml{kitdemo/fold2.sml}.}
\begin{verbatim}
       fun foldl f b xs = 
         let fun loop(p as ([], b))= p
               | loop(x::xs, b) = loop(xs,f x b))
...
\label{fold2.fig}
\end{figure}
There are two reasons why this version of {\tt foldl} is much better
than the straightforward version. First, the loop is implemented as a
jump and, more importantly, there is no new allocation in each
...
A project file has extension {\tt .pm}, and in it, one can mention a
...
with no imports (see e.g., Section~\ref{polyrec.sec} for such an
...
files were
recompiled.\index{recompilation}\index{project}
\index{source file}%
...
scanning example of Section~\ref{scan.sec}:\footnote{Project \boxml{kitdemo/scan.pm}.}
\begin{verbatim}
  lib.sml
  scan.sml
\end{verbatim}
...
The source files for the project are {\tt lib.sml} and and {\tt
  scan.sml}, which are both located in the directory where the project
file {\tt scan.pm} is located. Whereas each of the source files {\tt
...
The system detects automatically---from file modification dates---if a
source file is modified.  After a project has been successfully
compiled and linked, it can be executed by running the
command\index{run@{\tt run}}
\begin{verbatim}
  run
\end{verbatim}
...
variables. Thus, it might be the case that a source file is
...
\begin{itemize}
\item A \emph{region profile} is a graph
  \index{region profile}%
  which gives a ``global'' view of the memory usage by showing the
  total number of words allocated in regions and on the stack as a
  function of time. In the graph, regions that arise from the same
  \begin{center}
...
\item An \emph{object profile} is a graph
  \index{object profile}%
  which gives a ``local'' view into a particular region, as a function
  of time. The graph shows the objects allocated into a chosen region,
  with one coloured band for each allocation point in the
  region-annotated lambda program\footnote{Every occurrence of an
    ``\texttt{at}'' in the region-annotated lambda program is an
    allocation point.}.  Each allocation point is annotated with a
  \emph{program point}
  \index{program point}%
  which is a unique number identifying the
  allocation.\footnote{Program points are unique local to a project,
    e.g.\ with a project containing two source files, the program
    points in the region-annotated lambda programs for the two files
    will be distinct.}
...
\index{profile strategy!compile-time@compile-time}%
...
\index{generate lambda program with program points@\texttt{generate lambda program with pro\-gram points}}%
...
\index{generate region flow graph (.vcg file)@\texttt{generate region flow graph (.vcg file)}}%
...
\index{profile strategy!target@target}%
...
is specified by the \texttt{-sec n}\index{sec@\texttt{-sec} option}
and \texttt{-microsec n}\index{microsec@\texttt{-microsec} option}
