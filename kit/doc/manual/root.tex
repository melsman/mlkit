...
non-existing global region.\index{$\rhoword$}\index{boxing}\index{value!boxed}\index{value!unboxed} As an
...
and $\tau$ is a region-annotated type.\index{effect variable!bound}
...
\index{function type!region-annotated}\index{type!region-annotated}%
function type is
...
A \index{project}\index{project file}{\em project file} is a file which lists the SML source files
that make up a project. A project file can also 
\index{importing a project}{\em import} other
...
Project files have file extension \index{{\tt .pm}@pm}{\tt .pm}. In a project file, one
can refer to other project files and source files using  absolute
or relative \index{path!absolute}\index{path!relative}paths. 
...
style \index{project file!comment in}\index{comment!in project file}comments.
...
structures live at runtime.\index{structure declaration}
...
\section{Signatures}
Signature \index{signature declaration}declarations also exist at compile
...
declarations\index{signature constraint!transparent}\index{signature constraint!opaque}
\begin{verbatim}
