...
\end{verbatim}
Here \boxml{nthgen'} \index{nthgen@{\tt nthgen}}is the main loop of the program. It takes a pair as
argument; the first component of the pair indicates the number of iterations
desired, while the second, \boxml{g}, is the current generation. The use of
the {\tt as} pattern in line 1 forces the argument and the result of 
\boxml{nthgen'} to be in the same regions. Such a function is called
a \index{region endomorphism}{\em region endomorphism}. In line 3, we compute a fresh
...
because \boxml{nthgen'} is a region endomorphism (see Figure~\ref{doublecopy.fig}). 
...
{\tt kitdemo/life.sml}, \index{life@{\tt life}}project \boxml{kitdemo/life.pm}. (Running projects is
...
regions are \boxml{r1588} and \boxml{r1121}. To find out what these regions contain,
...
Every region has a \index{runtime type}runtime type. The following runtime types
...
The analysis which computes multiplicities is called the \index{multiplicity analysis}{\em multiplicity analysis}.
\end{description}
...
The Kit contains a \index{Lambda optimiser}{\em $\Lam$-optimiser}
...
The \index{runtime system}runtime system is written in C. It is small (less than 100Kb 
...
It is possible to call \index{C!calling}C functions from ML Kit code. The Kit takes care of the memory
...
allocated: it is just a constant which fits in a \index{register}KAM register.
...
\begin{verbatim}
       ...
       val x = #1 xy;
\end{verbatim}
Here is the resulting $\MulExp$ program:\footnote{Project: \boxml{kitdemo/proj.pm}, file \boxml{kitdemo/projection.sml}.}
\begin{verbatim}
...
\index{type!region-annotated}{\em region-annotated type}. The region-annotated type of an expression
is the ML type of the expression decorated with extra region information.
In a region-annotated type, every type constructor (e.g., \boxml{int},
\boxml{unit} and \boxml{list}) is paired with a region variable, indicating
where the value is going to be put at runtime.

The following are examples of region-annotated types
\begin{description}
\item[\fbox{$(\boxml{int}, \rho)$}] The type of integers in region $\rho$. 
\item[\fbox{$(\boxml{unit}, \rho)$}] The type of 0-tuples in region $\rho$. 
Integers and
0-tuples are represented \index{boxing}unboxed at runtime 
...
with one particular region variable,  \index{r2@{\tt r2}}\boxml{r2}. 
\item[\fbox{$\bigl((\boxml{int}, \rho_1)\ast (\boxml{string}, \rho_2), \rho_3\bigr)$}] Denotes
...
then selects a component of the pair, after which the pair is garbage:\footnote{Project: \boxml{kitdemo/effect.pm}, 
file: \boxml{kitdemo/elimpair.sml}.}
\begin{verbatim}
     val n = let 
...
The atomic effects on \index{r2@{\tt r2}}$\boxml{r2}$ stem from the integer operations: all integers
...
Note that the Kit \index{boxing}boxes tuples.
...
\label{tryit.sec}
The Kit is a \index{batch compilation}batch compiler.  A
\index{project}{\em project} consists of a number of SML source files,
enumerated in a \index{project file}{\em project file}. Executing a
...
To compile a project, you need your own personal \index{script
  file}{\em script file}. Your script file contains your personal
...
\texttt{Set project file name}; then type \boxml{"proj.pm"} (including
...
The \index{prelude}prelude must be
...
$f$\texttt{.sml} the Kit produces a \index{log file}{\em log file}
...
can now be done by selecting \boxml{Layout} from the main menu, and
then \texttt{print types}.

Thereafter, go back to the {\tt Project} menu, select {\tt Touch
  source file} and enter the string \boxml{"projection.sml"}, to
\index{touching a source file}indicate that you want to recompile that
...
\texttt{Set project file name}, enter \boxml{"effect.pm"}, and then
...
exercise, try executing the \index{hello world@{\tt hello world}}{\tt
  hello} project, which, like all other example files in this
document, is located in the \index{kitdemo directory@{\tt kitdemo}
  directory}{\tt kitdemo} directory.


\chapter{Basic Values}
\index{Standard ML!{1990}}\index{Standard ML!{1997 revision}}\index{Standard ML!{Basis Library}}
...
\section{Integers}
\label{integers.sec}
Values of type \index{integer}{\tt int} are represented as 32 bit integers.
The following operations on integers are 
pre-defined:\index{{\tt =}}\index{{\tt <>}}\index{{\tt <}}\index{{\tt >}}\index{{\tt <=}}\index{{\tt >=}}\index{{\tt +}}\index{{\tt -}}\index{div@{\tt div}}\index{mod@{\tt mod}}\index{{\tt *}}\index{\verb+~+}\index{abs@{\tt abs}}
\begin{verbatim}
infix  4 = <> < > <= >= 
infix  6 + - 
infix  7 div mod  * 
...
Integers are kept in KAM \index{register}registers or, when necessary, 
on the runtime \index{stack}stack. 
...
storing the values, a fixed region, \index{r2@{\tt r2}}{\tt r2}, 
is used throughout the program.

At present, arithmetic operations do not raise any exceptions when their
result is undefined or out of range.\index{exception}
\section{Reals}
The \index{real number}prelude defines the following operations on reals:
\index{{\tt =}}\index{{\tt <>}}\index{{\tt <}}\index{{\tt >}}\index{{\tt <=}}\index{{\tt >=}}\index{{\tt +}}\index{{\tt -}}\index{{\tt *}}\index{{\tt /}}\index{\verb+~+}\index{abs@{\tt abs}}\index{real@{\tt real}}\index{floor@{\tt floor}}\index{sqrt@{\tt sqrt}}\index{exp@{\tt exp}}\index{ln@{\tt ln}}\index{sin@{\tt sin}}\index{cos@{\tt cos}}\index{arctan@{\tt arctan}}
\begin{verbatim}
infix  4 = <> < > <= >= 
infix  6 + - 
infix  7  * /
...
\end{verbatim}
%All operations on reals are implemented by calls to C-functions in the
%runtime system.
Values of type {\tt real} are implemented as 64 bit floating point numbers. 
They are always boxed, i.e., represented as a pointer to two consecutive 32 bit \index{alignment}words.
These two words reside in a region and start on a double-aligned address. For that reason,
regions with \index{runtime type}runtime type {\tt real} (Section~\ref{runtimetypes.sec}) are never unified with regions of any
other runtime type.

A real constant $c$ in the source program is translated
into an expression of the form \index{at@{\tt at}}$c\at\rho$, where $\rho$ is a
...
result is undefined or out of range.\index{exception}
\section{Strings}
The prelude defines the following operations on strings:\index{{\tt =}}\index{\verb+^+}\index{ord@{\tt ord}}\index{chr@{\tt chr}}\index{size@{\tt size}}\index{explode@{\tt explode}}\index{implode@{\tt implode}}
\begin{verbatim}
infix  4 = 
infix  6 ^
val ord: string -> int
...
constructor and the pair into one value. %Indeed, it is possible to set parameters
%in the ML Kit to make it use this more compact list representation 
%(See Section~\ref{unboxlists.sec}).
%For this presentation, however, we stick with the scheme described above.
...
%
\put(80,0){\framebox(30,10){\boxml{nil}}}
...
Indeed a typical use of an infinite region is to hold the \index{spine of list}{\em spine} of
a list, i.e., all the {\tt ::} cells and the {\tt nil} cell of the list.\index{type!region-annotated}
...
In general, 
the region-annotated type and place of a list takes the form\index{type and place}
...
bound $\rho_1$, $\rho_2$ and $\rho_3$). The $\epsilon$ is a so-called \index{effect variable}{\em effect variable}. 
The 
\index{put@{$\Put$}}$\epsilon.\{\Put(\rho_3)\}$ appearing on the function arrow is called an \index{arrow effect}{\em arrow effect}.
