...
(* Modified for the RegExp modules distributed with the ML Kit. The
RegExp module is a modified version of the one distibruted with
Standard ML of New Jersey, 2001-08-07, Niels *)
...
(*
val _ = print(pp_mt "()(N)" "Niels" ^ "\n")

val _ = print(pp_mt "(ab)|(cd)" "xcdy" ^ "\n") (* Problem, cant distinguish *)
val _ = print(pp_mt "(ab)|(cd)" "xaby" ^ "\n") (* Problem, cant distinguish *)

val _ = print(pp_mt "((a(b+))+)" "babbabbb" ^ "\n") (* ok *)
Debug 2001-09-06, Niels *)

(* regcomp *)
val test1a = 
  tst' "test1a" (fn _ =>
		 let
		   (* All these patterns raises RegExp *)
		   val pats = ["", "(", "(()"] 
		 in
		   List.all (fn pat => (let val _ = regExpBool pat "dummy" (* We only test the pattern compilation *)
					in false end handle RegExp.RegExp _ => true)) pats
		 end)

(* regexecBool *)
val test2a = 
  tst' "test2a" (fn _ => 
		 List.all (fn (pat, ss) => not (List.exists (regExpBool pat) ss))
		 neg)

val test2b = 
  tst' "test2b" (fn _ => 
		 List.all (fn (pat, ss) => List.all (regExpBool pat) ss)
		 pos)

(* regexec *)
val test3a = 
   tst' "test3a" (fn _ => 
		 List.all (fn (pat, ss) => not (List.exists (Option.isSome o regExp pat) ss))
		 neg)

val test3b = 
  tst' "test3b" (fn _ => 
		 List.all (fn (pat, ss) => List.all (Option.isSome o regExp pat) ss)
		 pos)
