...
signature NS_DB =
  sig
    ...
    (* [panicDmlDb db f sql] same as dmlDb, except that on error
    function f is executed. panicDmlDb returns unit and it only
    raises an exception if f does. *)
    val panicDmlDb      : db -> (quot -> 'a) -> quot -> unit

    (* [panicDml f sql] executes sql and returns the value
    unit. On error the function f is applied to an error
    ...
    want the script to end on error, then f must call Ns.exit()
    as it's last expression.*)
    val panicDml      : (quot -> 'a) -> quot -> unit

    (* [dmlTransDb db f] executes function f using handle db,
    ...
    quotes: say v1 = qq v then qqq v = 'v1' *)
