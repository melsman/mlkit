...
begin
	...
	scs_test.printTest( 'new', 1, counter1 = 1 );  
	objid1 := scs.new_obj_id;
	roleid2 := scs_role.new( objid1, 'dba', textid1 );
	select count (*) into counter1 from scs_roles;
	scs_test.printTest( 'new', 2, roleid2 = objid1 and counter1 = 2 );
	...
	scs_test.printTest( 'add', 1, counter1 = 1 );
	scs_role.add( partyid0, roleid1 );
	select count (*) into counter1 from scs_role_rels;
	scs_test.printTest( 'add', 2, counter1 = 1 );

	-- testing has_p
	scs_test.printTest( 'has_p', 1, scs_role.has_p( partyid0, roleid1 ) = 't' );
	scs_test.printTest( 'has_p', 2, scs_role.has_p( partyid0, roleid2 ) = 'f' );
	
	-- testing remove
	scs_role.remove( partyid0, roleid1 );
	select count (*) into counter1 from scs_role_rels;
	scs_test.printTest( 'remove',1, counter1 = 0);
	scs_role.remove( partyid0, roleid1 );
	select count (*) into counter1 from scs_role_rels;
	scs_test.printTest( 'remove',2, counter1 = 0);
	...
	select count (*) into counter1 from scs_roles;
	scs_test.printTest( 'destroy', 1, counter2-2 = counter1 );  
end;
/
show errors
