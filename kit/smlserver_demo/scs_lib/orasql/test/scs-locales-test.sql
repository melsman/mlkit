...
begin
	...
	tid1 := scs_text.new;
	scs_test.printTest( 'new test1', (tid1 is null) = false );
	tid2 := scs_text.new ( tid1 );
	scs_test.printTest( 'new test2', tid1 = tid2 );

	tmp  := scs_text.updateText( tid1, 'da', 'kurt på dansk' );
	scs_test.printTest( 'updateText test1', tmp = tid1 );
	bool1 := scs_text.getText( tid1, 'da' ) = 'kurt på dansk';
	scs_test.printTest( 'updateText test2', bool1 );
	tmp  := scs_text.updateText( tid1, 'da', 'curt på dansk' );
 	scs_test.printTest( 'updateText test3', scs_text.getText( tid1, 'da' ) = 'curt på dansk' );
	tid3 := scs_text.updateText( language => 'da', text => 'niels på dansk',
				     language2 => 'en', text2 => 'niels in english' );  
	scs_test.printTest( 'updateText test4', 
	...
	scs_test.printTest( 'updateText test5', scs_text.getText( tid4, 'da' ) = 'sven på dansk' ); 
--        tmp  := scs_text.updateText( language => 'kd', text => 'fejl på kd'); -- should throw exception

	scs_test.printTest( 'getText test1', bool1 );
