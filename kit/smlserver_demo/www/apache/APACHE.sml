...
(*
 ...
 val scheduleWeekly  : string -> {day:int, hour:int, minute:int}
  ...
  (* sub-structures *)
  structure ApacheDebug  : APACHE_DEBUG
  structure Set      : APACHE_SET
  structure Conn     : APACHE_CONN where type status = status 
				 and type set = Set.set
  structure Cookie   : APACHE_COOKIE
  structure Cache    : APACHE_CACHE
  structure Info     : APACHE_INFO
  structure Mail     : APACHE_MAIL
  structure DbOra    : APACHE_DB where type set = Set.set
  structure DbPg     : APACHE_DB where type set = Set.set
  structure DbMySQL  : APACHE_DB where type set = Set.set
*)
