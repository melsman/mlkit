...
local
  ...
  fun show(x) = app (fn s => (Ns.Conn.write s; Ns.Conn.write "\n"))(plot(alive x));
  
  local 
    fun nthgen'(p as(0,g)) = p 
      | nthgen'(p as(i,g)) = 
          nthgen' (i-1, let val g' = nextgen  g
                        in  Ns.write `<h3>Generation Count Down: ^(Int.toString i)</h3><pre>`;
			    show g;
			    Ns.write `</pre>`;
                            copy g'
                        end)
...
in
  ...
  val _ = Ns.write `<html><body bgcolor=white><h2>Game of Life - let's play</h2>`
  val _ = go()
  val _ = Ns.write `<hr> <a href="mailto:smlserver@it.edu">smlserver@it.edu</a>
                    </body></html>`    
end
