...
functor DbOracleBackend(type conn = int
  ...
  struct 
    ...
    fun putHandle (h:DbHandle) : unit = 
               case !h of NONE => ()
                        | SOME  r => (log "apsmlDropSession" ; prim(":", ("apsmlDropSession",r : int,getReqRec()));
                                      h:= NONE)
    fun dmlDb (h : DbHandle) q : unit = 
       case !h of NONE => raise Fail "Session closed"
                | SOME r => let val  res : int = prim("@:", ("DBExecuteSQL",r : int, Quot.toString q : string, getReqRec()))
                            in if res <> 2
                               then (* not DBDml *) 
    ...
    fun execDb (h : DbHandle) q : unit = 
       case !h of NONE => raise Fail "Session closed"
                | SOME r => let val res : int = prim("@:", ("DBExecuteSQL",r,Quot.toString q, getReqRec()))
                            in if res = 0
                               then (* DBError *) 
    ...
    fun selectDb (h : DbHandle) q = 
       case !h of NONE => raise Fail "Session closed"
                | SOME r => 
        let val res : int = (log "DBExecuteSQL" ; prim("@:",("DBExecuteSQL",r, Quot.toString q, getReqRec())))
              val msg = Quot.toString (`selectDb: SQL Error on '` ^^ q ^^ `'`)
          in case res of 1 => (*DBData*) 
                             (let val res2 : string list = (log "apsmlGetCNames" ;prim(":", ("apsmlGetCNames",r,getReqRec())))
    ...
    fun getRowListDb (h,f,l) = case !h of NONE => raise Fail "Session is closed"
                                        | SOME r => 
                             let val (res,res2) : ((string * int) list * int) = (log "apsmlGetRow" ; prim(":", ("apsmlGetRow",r,getReqRec())))
                             in 
    ...
    fun dmlTransDb h f = case !h of NONE => raise Fail "Session is closed"
                                  | SOME r => let val _ = log("TransStart")
                                                  val res : int = prim("@:", ("DBTransStart",r : int))
                                              in if res = 0 
                                                 then raise Fail "Transaction already started"
                                                 else 
                    (((f h) handle X => (log "TransRollBack"; prim("@:", ("DBTransRollBack",r : int, getReqRec())) : int; raise X)) before ( 
                         case !h of NONE => raise Fail "Session closed prematurely"
                                  | SOME r2 => let val _ = log "TransCommit"
                                                   val res2 : int = prim("@:", ("DBTransCommit",r : int,getReqRec()))
                                               in if res2 <> 2 (* DBDml *) 
