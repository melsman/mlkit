...
(* 
 ...
 entries. The following strategies are supported:
     ...
     * WhileUsed t : elements are emitted from the cache after
       approximately t time after the last use.

     * TimeOut t : elements are emitted from the cache after
       approximately t time after they are inserted.

     * Size n : the cache has a maximum size of n bytes.
       Elements are emitted as needed in order to store new
       elements.  The size n should not be too small, a
       minimum size of 1 Kb seems to work fine for small
    ...
    described by ck.
     ...
     AOLserver does not support arbitrary long cache names.
     The cache name you provide is only part of the
     constructed name that we use in AOLserver (i.e., we
     append typing information to the name you provide in
     order to maintain type safety). The function get raises
     exception Fail in case the constructed cache name is too
     large for AOLserver. This issue is solved in future
     (version 4) releases of AOLserver.

    [lookup c k] returns the value associated with the key k
    in cache c; returns NONE if k is not in the cache.

    [insert (c,k,v)] associates a key k with a value v in the
    ...
    [memoize c f] implements memoization on the function f.
    The function f must be a mapping of keys and elements that
    can be stored in a cache, that is, f is of type 'a Type ->
    'b Type.

    [memoizePartial c f] memoizes function values y where f
    returned SOME y
