(*$IntFinMap: PRETTYPRINT REPORT ORDER_FINMAP*)
...
(* for profiling:
    fun depth E = 1
      | depth (N(_,_,t1,t2,_)) =
         let val n1 = depth t1
             val n2 = depth t2
         in  1+(if n1>n2 then n1 else n2)
         end

    val trace = ref [] : int list ref
*)
	...
	let val result = insert((k0, d0), t) 
                         	handle ALREADYTHERE => update ((k0, d0), t)
        in
           (*trace:= depth result :: !trace; *)
           result
        end
    end
