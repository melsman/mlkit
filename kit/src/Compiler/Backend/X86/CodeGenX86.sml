...
(*
    ...
    local
      val static_data : inst list ref = ref []
    in
    ...
    fun set_atbot_bit(dst_ea:ea,C) = orl(dst_ea, I 2) (*00010*)
    fun clear_atbot_bit(dst_reg:reg,C) = DEPI{cond=NEVER,i="0",p="30",len="1",t=dst_reg} :: C
    fun set_inf_bit(dst_ea:ea,C) = orl(dst_ea, I 1) (*00001*)
...
*)
  in
    fun CG {main_lab:label,
	    ...
	    safe:bool} =  {top_decls=[], init_code=[], exit_code=[], static_data=[]}
(*
      let
...
*)
    ...
    (* ------------------------------------------------------------------------------ *)
    fun generate_link_code (linkinfos:label list) : I.AsmPrg = {top_decls=[], init_code=[], exit_code=[], static_data=[]}
(*
      let	
	val _ = reset_static_data()
	val _ = reset_label_counter()
...
(*      HpPaRisc.output_AsmPrg(os,prg);
      TextIO.closeOut os;
      TextIO.output(TextIO.stdOut, "[wrote X86 code file:\t" ^ filename ^ "]\n") *)
    end 
