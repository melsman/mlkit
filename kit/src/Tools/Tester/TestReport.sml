signature TEST_REPORT =
  sig
    ...
    val reset : unit -> unit
    val export : {errors:int, testfile_string:string} -> unit
...
structure TestReport : TEST_REPORT =
  struct
    ...
    fun kitversion() =
      if OS.Process.system "kit -version > KITversion" = OS.Process.success then
	read_all "KITversion"
      else "failure while executing `kit -version'\n"
    ...
    fun export {errors:int, testfile_string: string} : unit =
	  ...
	  fun abstract() =
	    (beginenv "abstract";
	     ...
	     outln "Here is the output from executing {\\tt kit -version}:";
	     beginenv "quote";
	     beginenv "verbatim";
	     out (kitversion());
	     endenv "verbatim";
