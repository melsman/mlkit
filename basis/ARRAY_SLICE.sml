...
length sl
    ...
    returns |sl|, the length (i.e., number of elements) of the
...
slice (arr, i, sz)
    ...
    array. If sz is NONE, the slice includes all of the elements to
    the end of the array, i.e., arr[i..|arr|-1]. This raises Subscript
    if i < 0 or |arr| < i. If sz is SOME(j), the slice has length j,
    that is, it corresponds to arr[i..i+j-1]. It raises Subscript if i
    < 0 or j < 0 or |arr| < i + j. Note that, if defined, slice
...
subslice (sl, i, sz)
    ...
    or j < 0 or |sl| < i + j. Note that, if defined, subslice returns
...
vector sl
    ...
    generates a vector from the slice sl. Specifically, the result is
    equivalent to
...
copyVec {src, dst, di}
    ...
    These functions copy the given slice into the array dst, with the
    i(th) element of src, for 0 <= i < |src|, being copied to position
...
app f sl
    ...
    These functions apply the function f to the elements of a slice in
    order of increasing indices. The more general appi function
...
modify f sl
    ...
    result. The more general modifyi supplies f with the index of the
...
foldr f init sl
    ...
    These functions fold the function f over the elements of a slice,
...
find f sl
    ...
    These functions apply f to each element of the slice sl, in order
    of increasing indices, until a true value is returned. If this
    occurs, the functions return the element; otherwise, they return
    NONE. The more general version findi also supplies f with the
    index of the element in the slice and, upon finding an entry
    satisfying the predicate, returns that index with the element.

exists f sl

    applies f to each element x of the slice sl, in order of
    increasing indices, until f x evaluates to true; it returns true
