...
struct
  ...
  struct
    ...
    datatype killpid_arg = K_PROC of pid
                          | K_SAME_GROUP
                          | K_GROUP of pid

    datatype waitpid_arg = W_ANY_CHILD | W_CHILD of pid | W_GROUP of pid | W_SAME_GROUP
  ...
  struct
	...
	let
	  val (utime_sec : int,
          ...
          val {elapsed_sec : int,
               elapsed_usec : int} = prim("sml_getrealtime", ())

          (*	  val cps = SysWord.toInt(sysconf "CLK_TCK")
	  fun split t = (t div cps, Real.floor(1000000.0 * Real.fromInt(t mod cps)) div cps)
*)
	  fun toTime (sec,usec) = Time.+(Time.fromSeconds (Int.toLarge sec),
					 Time.fromMicroseconds (Int.toLarge usec))
	in
	  {elapsed = toTime (elapsed_sec, elapsed_usec),
	   utime = toTime (utime_sec, utime_usec),
	   stime = toTime (stime_sec, stime_usec),
	   cutime = toTime (cutime_sec, cutime_usec),
	   cstime = toTime (cstime_sec, cstime_usec)
          }
	end
