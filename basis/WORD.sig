...
[toLargeX w] These convert w to a value of type LargeWord.word. In the
...
[fromLargeWord w] These functions convert w to the value w(mod
...
[toLargeIntX w] These convert w to a value of type LargeInt.int. In
...
[fromLargeInt i] converts i of type LargeInt.int to a value of type
word. This has the effect of taking the low-order wordSize bits of the
2's complement representation of i.

[toInt w]

[toIntX w] These convert w to a value of default integer type. In the
...
[fromInt i] converts i of the default integer type to a value of type
word. This has the effect of taking the low-order wordSize bits of the
...
[>> (i, n)] shifts i to the right by n bit positions, filling in zeros
...
[~>> (i, n)] shifts i to the right by n bit positions. The value of
...
[i + j] returns (i+j)(mod (2 (wordSize))) when i and j are interpreted
as unsigned binary numbers. It does not raise Overflow.

[i - j] returns the difference of i and j modulo (2(wordSize)):
...
[i * j] returns the product (i*j)(mod (2(wordSize))) when i and j are
interpreted as unsigned binary numbers. It does not raise Overflow.

[i div j] returns the truncated quotient of i and j, floor((i / j)),
...
[compare (i, j)] returns LESS, EQUAL, or GREATER if and only if i is
...
[toString i] These return a string containing a numeric representation
...
[fromString s] These functions scan a word from a character source. In
