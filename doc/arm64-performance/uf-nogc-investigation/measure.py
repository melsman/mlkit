...
r=Path('/private/tmp/mlkit-uf-nogc');variants=['x64','arm64','direct','result','inline'];samples={a:[] for a in variants};rng=random.Random(223);expected=(r/'arm64/uf.mlb.out.ok').read_bytes()
for rep in range(8):
...
(r/'measurements.json').write_text(json.dumps({'samples':samples,'medians':medians,'sha256':{a:hashlib.sha256((r/(a+'.exe')).read_bytes()).hexdigest() for a in variants}},indent=2)+'\n')
