...
for name, source in [('mlyacc', 'mlyacc.mlb'), ('professor', 'professor.sml')]:
    ...
    executables = {key: root / (name + '-gc.exe')
                   for key, root in [('old', args.old), ('wrappers', args.wrappers), ('both', args.new)]}
...
args.output.write_text(json.dumps({
    'baseline_revision': '99f760e',
    'variants': {'old': 'live-register preservation',
                 'wrappers': 'identity tail wrappers; -no_arm64_self_loops',
                 'both': 'identity tail wrappers and eligible self loops'},
