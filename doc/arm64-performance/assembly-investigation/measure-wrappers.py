...
for name,src in [('mlyacc','mlyacc.mlb')]:
 ...
 variants={'fresh_basis':root/(name+'-gc.exe'),'wrappers':root/(name+'-wrappers.exe')}
 ...
 (root/'wrapper-measurements.json').write_text(json.dumps({'revision':'07cf1e8','warmups':1,'repetitions':5,'order':'seeded shuffle per round','runtime_flag':'-report_gc','variants':'fresh Basis rebuilt by current MLKit; isolated assembly experiments affect benchmark units only','results':results},indent=2)+'\n')
