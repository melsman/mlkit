...
for name,src in [('mlyacc','mlyacc.mlb')]:
 exe={**{v:root/('mlyacc-'+v+'.exe') for v in ['baseline','leaf','inline','comparisons','constants']}}
 expected=(Path('/private/tmp/mlkit-m12-return-fix/after/bench')/(src+'.out.ok')).read_bytes()
 ...
 for rep in range(6):
  ...
  for variant in order:
   ...
   p=subprocess.run([str(exe[variant]),'-report_gc'],cwd=Path('/private/tmp/mlkit-m12-return-fix/after/bench'),capture_output=True,timeout=60)
...
(root/'measurements.json').write_text(json.dumps({'production_revision':'a6d7fb9','gc':True,'warmups':1,'repetitions':5,'basis':'All ARM variants share M12RetAfter Basis/runtime; benchmark assembly only changes','results':results},indent=2)+'\n')
