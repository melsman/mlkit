...
for name,src in [('mlyacc','mlyacc.mlb'),('professor','professor.sml')]:
 exe={'x64':Path('/private/tmp/mlkit-m11-final')/(name+'-x64-gc.exe'),'before':Path('/private/tmp/mlkit-m12-fresh-basis')/(name+'-gc.exe'),'after':root/(name+'-gc.exe')}
 ...
 for rep in range(6):
  ...
  for variant in order:
   ...
   p=subprocess.run([str(exe[variant]),'-report_gc'],cwd=root/'bench',capture_output=True,timeout=60)
...
(root/'measurements.json').write_text(json.dumps({'before_revision':'ad5a7a8','gc':True,'warmups':1,'repetitions':5,'basis':'Before: M12FreshFrames; after: fresh M12SelectFinal; both fully rebuilt using their corresponding compilers','results':results},indent=2)+'\n')
