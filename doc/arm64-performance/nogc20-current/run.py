...
ROOT=Path('/private/tmp/mlkit-nogc20-current');REPO=Path('/private/tmp/mlkit-m2-source')
...
def build():
 ...
 for a in ['arm64','x64']:assert not list((REPO/'basis').glob('MLB/*NoGC20Current'+a)),a
 for key,p in SOURCES.items():shutil.copytree(p,ROOT/key,ignore=shutil.ignore_patterns('.git','MLB'),dirs_exist_ok=False)
 meta={'revision':git(REPO),'source_revisions':{k:git(p) for k,p in SOURCES.items()},'platform':platform.platform(),'date':'2026-09-23','gc':False,'warmups':1,'repetitions':5,'memory_metric':'wait4 rusage.ru_maxrss, Darwin bytes, peak process RSS','timing_metric':'perf_counter elapsed seconds, separate process, includes startup and I/O','compiler_sha256':{a:sha(ROOT/('mlkit-'+a)) for a in ['arm64','x64']},'runtime_sha256':{a:sha(REPO/'lib'/a/'runtimeSystem.a') for a in ['darwin-arm64','darwin-x86_64']}}
 rows=[]
 for name,cwd,src in specs():
  expected=cwd/(src+'.out.ok')
  if not expected.exists():expected=cwd/(src.lower()+'.out.ok')
  for arch in ['x64','arm64']:
   exe=ROOT/(name+'-'+arch+'.exe');cmd=[str(ROOT/('mlkit-'+arch)),'-no_gc','--mlb-subdir','NoGC20Current'+arch,'-o',str(exe)]
   ...
   symbols=subprocess.check_output(['nm','-g',str(exe)],text=True)
   assert not any(line.split()[-1:] in [['_gc'],['_time_to_gc']] for line in symbols.splitlines()),str(exe)
  ...
  for rep in range(6):
   ...
   for row in order:
    ...
    with out.open('wb') as stdout,err.open('wb') as stderr:
     start=time.perf_counter();p=subprocess.Popen([row['exe']],cwd=row['cwd'],stdout=stdout,stderr=stderr)
     def kill():
