...
r=Path('/private/tmp/mlkit-m12-mlyacc-study');links={}
...
for name,row in links.items():
 ...
 for i,a in enumerate(args):
  ...
  if not(str(obj).startswith('/private/tmp/mlkit-m12-select/bench') or obj.name=='List.sml.o'):continue
  s=asm.read_text()
 ...
 expected=(Path(row['cwd'])/('mlyacc.mlb.out.ok' if name=='mlyacc' else 'professor.sml.out.ok')).read_bytes();assert p.stdout==expected
