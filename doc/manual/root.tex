...
follows:\index{rp2ps@{\tt rp2ps}}\footnote{The program {\tt rp2ps} can
...
Figure~\ref{lifeprof80.fig} was produced by first\index{LaTeX@{\LaTeX\ document!including figure in}}
...
A \index{record!runtime representation of}record with 0 components
...
Booleans are unboxed. The initial basis\index{initial basis} provides
...
bound region variables $\rho_1$ and $\rho_2$). The $\epsilon$ is a so-called \index{effect variable}{\em effect variable}. 
...
type scheme}region type schemes that the region inference algorithm
...
to the former functions as \index{region exomorphism}{\em region
  ...
  region endomorphisms}. Notice that this is not a clear-cut 
...
ideas is listed in Figure~\ref{msort.fig}.\index{cp@{\tt
    cp}}\index{msort@{\tt msort}}\index{merge sort}\index{projects!compiling}\index{projects!running}\footnote{Project
  ...
  return. Finally, choose {\tt Compile and link project}. The Kit places an
  executable file \boxml{run} in the
  \boxml{kitdemo} directory. For an in-depth description of how to
  compile and run projects, see Chapter~\ref{modules_and_projects.chap}.}
\begin{figure}[t]
\hrule\medskip
\begin{verbatim}
       fun cp [] =[]
...
\index{declaration!of value}declarations so as
...
will create a list using fresh regions. Because {\tt cp} is also \index{region exomorphism}exomorphic,
...
Useful \index{program transformation}program transformations to
...
Standard ML
\index{exception}\index{exception constructor}%
exception constructors are introduced by
\index{exception declaration}\index{exception@{\tt exception}}% 
...
fresh
\index{exception!generative}\index{exception name}%
...
\index{allocation point}\index{at@{\tt at}}%
...
called \index{storage mode}{\em storage modes}.
...
region.\footnote{On machines\label{atbit.lab} that have at least four
  ...
  \index{region name}region name.  One bit, called the ``atbot bit'',
...
the flag
\index{print drop regions with storage modes@{\tt print drop regions with storage modes}}
...
\index{variable!locally live}{\em locally live variables}, that is, a set
...
that appear in function application expressions.) Prior to the
...
has made sure that every intermediate result that arises during
...
extra {\tt let} bindings, when necessary.)\footnote{K-normalisation is
  ...
  programs are always simplified to eliminate the extra {\tt let} bindings
...
or {\tt attop} in case (B3), the Kit builds a \index{region flow
  graph}\label{region flow graph}{\em region flow graph\/} for the
...
replaces (B3) is:\index{region parameter!formal}
\begin{description}
...
Next, {\tt klength} is a version that contains a local \index{region endomorphism}region 
...
\item The iteration should be done by an auxiliary, uncurried function
  ...
  (informally) to this auxiliary function as the \index{iterator}{\em
    iterator}.
\item The iterator should be a \index{region endomorphism}region
...
$$\resetr\; {\it vid}$$
and \index{forceResetting@{\tt forceResetting}}
$$\resetf\; {\it vid}$$
...
\index{merge sort}\index{msort@{\tt msort}}%
...
was generated by the \index{sampleMax@{\tt sampleMax}}\index{eps file}\index{rp2ps@{\tt rp2ps}}command \boxml{rp2ps -region -sampleMax 1000 -eps 137 mm}
...
\index{region polymorphism} region-polymorphic.\footnote{The reason
  for this is that the expression on the right-hand side of the value
  declaration might have an effect (e.g, print something) before
  returning the function.  It would not be correct to suspend this
  effect by introducing formal region parameters.} However, in the
special case where the right-hand side of the value declaration is a
\index{lambda abstraction}lambda abstraction, the Kit automatically
converts the declaration into a {\tt fun} declaration, thereby making
...
Although enabling these flags gives very verbose output, it is
instructive to look at such a term just once, to see how arrow effects
are instantiated. We show the full output for the {\tt apply} example
in Figure~\ref{apply.fig}. 
...
to determine what function calls are tail calls and, more generally, what function calls fall
into the four special cases. We say that expressions produced
...
in the menu~~ \texttt{Printing of intermediate forms}, and thus check whether specific function calls in the code turn out the way one intended.
...
a function call as a simple \index{jump}\index{jmp@{\tt
    jmp}}\index{region polymorphism}jump. A call of a
region-polymorphic function takes the form $$\boxml{$f$ [$\rho_1$,
  $\ldots$, $\rho_n$] $\at\,\rho_0$ $\exp$}$$ where $\rho_0$ is the region
that holds the region vector containing the actual region parameters
$\rho_1$, $\ldots$, $\rho_n$.  During
...
generator\index{graph generator@\texttt{rp2ps} options|(} \texttt{rp2ps} (read: RegionProfile2PostScript) found in the
...
write \index{convertIntToC@\texttt{convertIntToC}}
\begin{verbatim}
