...
% Do not include the page in comments below; it will cause the document to be aligned on odd pages, which is not
% good for printing.
%\setcounter{page}{0}
%\thispagestyle{empty}
%\begin{center}{\bf
...
\index{region stack}%
stack of \index{region}{\em regions}, see Figure~\ref{stacks.fig}.
\begin{figure}[t]
\hrule
\begin{center}
...
regions are \boxml{r94613} and \boxml{r94231}. To find out what these regions contain,
...
\end{verbatim}
The option \boxml{-sampleMax $n$} instructs \boxml{rp2ps} 
...
\index{eps@{\tt -eps} option}%
\index{sampleMax@{\tt -sampleMax} option}%
\index{region@{\tt -region} option}%
\begin{verbatim}
 rp2ps -region -name life -sampleMax 80 -eps 137 mm
\end{verbatim}
The option \boxml{-eps 137 mm} has the effect that \boxml{region.ps}
...
\label{boxing.sec}
\index{boxing}\index{value!boxed}\index{value!unboxed}
...
are part of a boxed value. For example, the integer \boxml{3} by itself
is stored as the (binary representation) of the value 3 in a
KAM register. However, the pair \boxml{(3,4)} is represented as a
...
allocated; it is just a constant that fits in a \index{register}KAM
register.
...
\item The expression \verb+{|xy: (_,r1), x: _|}+ is
    ...
    % are not allocated in regions.)
      ...
      evaluated,  \boxml{xy} will reside in \boxml{r1}.
...
region.\index{$\rhoword$}\index{boxing}\index{value!boxed}\index{value!unboxed}
...
\item[\fbox{$\boxml{unit}$}] The type of 0-tuples.  Integers,
  booleans, and 0-tuples are represented \index{boxing}unboxed at
...
when it is safe to put \boxml{letregion} around  an expression.\index{letregion@{\tt letregion}}
...
not just a region-annotated type with place, but also a so-called \index{effect}{\em effect}. 
...
(the value of type \index{unit@{\tt unit}}{\tt unit}) is represented
...
\index{equality!polymorphic}polymorphic equality is compiled into
\index{equality!monomorphic}monomorphic equality functions that do not
...
The 
\index{initial basis}%
initial basis provides the following top-level operations on characters and strings:\index{{\tt =}}\index{\verb+^+}\index{ord@{\tt ord}}\index{chr@{\tt chr}}\index{str@{\tt str}}\index{size@{\tt size}}\index{explode@{\tt explode}}\index{implode@{\tt implode}}\index{concat@{\tt concat}}\index{substring@{\tt substring}}
\begin{verbatim}
infix  4 = 
infix  6 ^
val ord: char -> int
...
booleans:\index{{\tt =}}\index{not@{\tt not}}
\begin{verbatim}
infix 4 =
val not: bool -> bool
\end{verbatim}
...
from the auxiliary pair to which \boxml{::} is applied. Since \boxml{nil} is
represented by an odd number and since word addresses are always even, \boxml{nil}
...
or in applying the decomposition operator \boxml{decon\_::} to a non-empty list.
...
Note that the \boxml{list} type constructor is not paired with a region variable.
...
In this chapter, we shall treat \index{function}\index{function!first-order}functions that are declared with
\index{fun@{\tt fun}}{\tt fun} and that are first-order (i.e., that neither take functions
...
\end{tabbing}
where \boxml{[$\rho_1'$, $\cdots$, $\rho_k'$]} is a record of 
\index{region parameter!actual}%
...
do not use brackets for lists.\index{[ ]@{\tt [ ]}}.

In the special case $k'=0$ the record for actual region parameters is empty
and is therefore not allocated. We therefore omit printing the ``\boxml{at $\rho_0$}''
in that case.
...
is region-polymorphic. (Program: \boxml{kitdemo/fromto.sml}, printed by selecting
...
\label{fromto.fig}
\end{figure}
Notice that \boxml{r7} is a formal region parameter of {\tt fromto}.\index{fromto@{\tt fromto}}
In the last call of {\tt fromto}, a record holding a
region descriptor for \boxml{r19} is passed to {\tt fromto};
...
trouble to distinguish between $\Put$ and \index{get@{$\Get$}}$\Get$
...
Thus, \boxml{ys} and \boxml{xs @ ys} must be in the same regions. However, the auxiliary pairs of
...
\begin{verbatim}
       ...
       fun cp2 (l as []) = l
         | cp2 (x::xs) = x :: cp2 xs
\end{verbatim}
Here \boxml{cp1} can copy the auxiliary pairs of a list into a fresh region, whereas \boxml{cp2} always
...
type with place, \boxml{l} and \boxml{cp1 l} are forced to be in the same regions.
%
...
\index{cp@{\tt cp}}\index{msort@{\tt msort}}
\index{merge sort}%
\index{projects!compiling}\index{projects!running}\footnote{Project
  {\tt kitdemo/msort.pm}, file {\tt kitdemo/msort.sml}. To compile the
  project, enter the \boxml{Project} sub-menu (from within the
  \boxml{kitdemo} directory) and choose {\tt Set project file name}.
...
Because \boxml{merge} is not tail-recursive, 
...
each recursive call of \boxml{merge} allocates two words
...
\boxml{merge} calls \boxml{cp}, which allocates less for each 
iterative call than \boxml{merge} does. Each return from 
\boxml{merge} allocates a list cell (two words) but deallocates
...
It turns out that each iterative call of \boxml{merge} pushes
...
pop the stack. That this idea works in 
\index{block structure}%
block-structured languages hinges
...
Rewriting the applicationn of \boxml{sieve} ensures that the list bound to
...
Useful \index{program transformation}program transformations 
include:\index{lifetime|)}\index{lifetime!shortening}
\begin{enumerate}
...
in connection with \index{tail recursion}tail recursion;
...
source program\footnote{Program \boxml{kitdemo/escape.sml}.}
\begin{verbatim}
       fun g() = 
         let val x = [5,7]
...
\end{verbatim} 
Here \boxml{f} has type $\boxml{int}\to\boxml{int}$; yet, when the expression \boxml{y>3} evaluates
to \boxml{true}, an append operation is performed that produces a list in the same region
...
inlining in the {\Lam} optimiser; this is done by setting the {\tt
    Maximum inline size}, found in the 
        {\tt Control/Optimiser} sub-menu, to {\tt 0}.\index{optimiser}}
\begin{figure}
\hrule
\begin{verbatim}
...
leak in region \boxml{r8}. Looking at the 
function \boxml{f}, we see that this region is an actual
region parameter to \boxml{@}. It follows that the problem is the
call to \boxml{@}.
...
we can move the construction of the list into \boxml{f}:\footnote{Program \boxml{kitdemo/escape1.sml}.}
\begin{verbatim}
       fun g() = 
         let fun mk_x() = [5,7]
...
the calls as possible and then pass the list as an extra argument:\footnote{Program \boxml{kitdemo/escape2.sml}.}
\begin{verbatim}
       fun g() = 
         let 
...
\boxml{f}. This quantification has two advantages: (a) allocation of space for the list is
...
free in the declaration of \boxml{f} and will be kept alive as long as \boxml{f} can be called).
...
Basis Library:\index{TextIO@{\tt TextIO}}\index{openIn@{\tt
    openIn}}\index{openOut@{\tt openOut}}
\begin{verbatim}
...
\end{verbatim}
Because \boxml{!y} evaluates to true, {\tt z} becomes bound
...
$v$ may be a \index{boxing}boxed or an unboxed value.  Next, a 32-bit
...
of the declaration ~\boxml{val x = ref $\at\,\rho$ 3}~ results in the environment
$\{\boxml{x}\mapsto r\}$, where $r$ is the address of a word with
contents 3 residing in region \boxml{r35}, see Figure~\ref{refs.fig}.
The type of \boxml{x} is {\tt ((int,$\rhoword$) ref, $\rho$)},  which, as usual,
we shorten to \boxml{(int ref, $\rho$)}.
...
:=  & \forall\alpha\rho_1\rho_2\rho_3\epsilon.(((\alpha,\rho_1)\REF,\rho_2)\ast(\alpha,\rho_1),\rho_3)
           \ar{\epsilon.\{\Get(\rho_3),\Put(\rho_2)\}} \UNIT\cr}
\medskip
...
program shown in Figure~\ref{otherrefs.fig}.\footnote{Program \boxml{kitdemo/refs3.sml}.}
\begin{figure}
\hrule
\begin{verbatim}
...
that bind \boxml{r7} and \boxml{r8}.

\section{Local References}
References \index{reference!local} that are created locally within a function and that do
not escape the function naturally reside in regions that are local to
the function body.
For example, the declaration:\footnote{Program \boxml{kitdemo/refs1.sml}.}
\begin{verbatim}
...
the reference is an {\em own variable} \index{variable!own}of the function.)\footnote{Program \boxml{kitdemo/refs2.sml}.}
\begin{verbatim}
       local
...
%follows:\index{tree@{\tt tree}}\index{Lf@{\tt Lf}}\index{Br@{\tt Br}}\index{datatype@{\tt datatype}}
%\begin{verbatim}
...
\index{tree@{\tt tree}}\index{Lf@{\tt Lf}}\index{Br@{\tt Br}}\index{datatype@{\tt datatype}}
\begin{verbatim}
       datatype 'a tree = Lf | Br of 'a * 'a tree * 'a tree;
\end{verbatim}
...
\end{verbatim}
Here one needs an arrow effect for the function type \boxml{V -> V}. We refer
...
This is why we put all exception values in
\index{region!global}%
global regions.
...
\item checking applications of $\resetr$ (and \index{forceResetting@$\resetf$}$\resetf$) so as to 
...
application could still not use $\atbot$, for \boxml{l1} is
...
holds \boxml{l1}, for by that time, \boxml{l1} is
...
the flag
\index{print drop regions expression with storage modes@{\tt print drop regions expression with storage modes}}%
...
\index{K-normalisation}\label{K-normal-form}{\em K-normalisation}, has
...
abstraction, 
that is, inside an expression of the form \boxml{fn {\it pat} => $e$}.
...
\item[B2]{\it $\rho$ is bound by a {\tt letregion} expression inside the
    ...
    \item[B3 (first attempt)]{\it $\rho$ is a formal parameter of a
        ...
        smallest enclosing lambda abstraction:} use \index{sat@{\tt
          sat}}{\tt sat}, if no locally live variable at the
...
variable at the allocation point has $\rho$ free in its region-annotated
type scheme with place, and use {\tt attop} otherwise.
...
Consider the functions declared in Figure~\ref{length.fig};\footnote{Program \boxml{kitdemo/length.sml}.} they
...
Next, {\tt klength} is a version that contains a local 
\index{region endomorphism}%
...
declared outside {\tt llength}, using \index{local@{\tt local}}{\tt
  local}.
\begin{figure}
\vbox{
\hrule
...
\item The iteration should be done by an auxiliary, uncurried function
  ...
  (informally) to this auxiliary function as the \index{iterator}{\em
    iterator}.
...
\index{resetRegions@{\tt resetRegions}}
$$\resetr\; {\it vid}$$
...
one sees that \boxml{r58} is a formal parameter of
{\tt msort}.  Due to the recursive call {\tt msort l}, the region
graph contains an edge from \boxml{r58} to \boxml{r65}. Thus the
...
see Figure~\ref{force.fig}.\footnote{Project:   \boxml{kitdemo/msortreset2.pm}, file
...
\index{eps file}%
\index{rp2ps@{\tt rp2ps}}%
command \boxml{rp2ps -region -sampleMax 1000 -eps 137 mm}
...
\caption{Region profile of the scanner. The unit of measure on 
  ...
  command \boxml{run -microsec 100000 < ../../kitdemo/scanfiles}. A
  PostScript file \boxml{region.ps} can be generated with the command
  \boxml{rp2ps -region -sampleMax 1000 -eps 137 mm}.  }
\label{scan.fig}
\end{figure}
%
%
...
\index{function!higher-order}in Standard ML is an expression of the
form 
...
{\it exp}.\index{variable!lambda-bound} 
...
declared with \index{val@{\tt val}}{\tt val} do not in general become
\index{region polymorphism}%
...
Functions declared using this abbreviation are said to be \index{function!Curried}{\em Curried}. 
...
knowing the lifetimes of closures.\footnote{Program \boxml{kitdemo/lambda.sml}.} Consider
\begin{verbatim}
...
function \boxml{(fn ys => $\cdots$)} may be called, for this function
...
Notice that \boxml{r10} occurs free in the type of the
...
dependencies between functions. The following example illustrates the point:\footnote{Program \boxml{kitdemo/apply.sml}.}
\begin{verbatim}
       fun apply f x = f x
...
\end{tabbing}
Formally, a \index{substitution}{\em substitution\/} is a triple
...
\caption{The instantiation of arrow effects keeps different applications of
  ...
  was obtained by compiling the program \boxml{kitdemo/apply.sml} with
  {\tt Control/Optimiser/maximum inline size} menu entry set to {\tt 0} and with the flags \boxml{print types} and
  \boxml{print effects} enabled.}  \medskip \hrule
\label{apply.fig}
\end{figure}
...
nodes.  For instance, \boxml{e13(U(U,get(r8),e11))} in the figure
...
second use of {\tt apply}, \boxml{r8} was instantiated to {\tt r26}.
...
all the uses of $\boxml{f}$ use the same regions. Indeed, because \boxml{f} occurs
...
function (i.e., \boxml{r27}) is allocated prior 
...
\end{verbatim}
Because {\tt f} is lambda-bound, all the pairs created by the expression \boxml{(x,acc)}
...
the region of the pair is free in the region-annotated type of \boxml{f},
...
\index{print call-explicit expression@{\tt print call-explicit expression}}%
...
There is one (and so far only one) \index{register}register that is used for passing arguments
...
a function call as a simple \index{jump}\index{jmp@{\tt
    jmp}}
\index{region polymorphism}%
...
\index{K-normalisation}K-normalisation, the Kit tries to bring the
...
a region vector containing \boxml{r} has to be constructed, because,
...
The following is an example of how one obtains simple jumps:\footnote{Program \boxml{kitdemo/tail.sml}.}
\begin{verbatim}
       local
         fun f'(p as (0,b)) = p
           | f'(n,b) = f'(n-1,n*b)
...
register called \index{standardClos@{\tt standardClos}}\index{register}
...
The Basis Library declares function composition as follows\footnote{Program \boxml{kitdemo/compose.sml}.}
\begin{verbatim}
       fun (f o g) x = f(g x)
\end{verbatim}
...
\end{verbatim}
Notice that \boxml{f o g} first creates a closure in \boxml{r7}
...
\label{foldl.sec}
Consider the following declaration of folding over lists:\footnote{Program \boxml{kitdemo/fold1.sml}.}
\begin{verbatim}
       fun foldl f b xs = 
         case xs of 
           [] => b
...
\end{verbatim}
The recursive call of \index{foldl@{\tt foldl}}{\tt foldl} is a call of a known function, but not a tail
...
{\tt f} takes a pair as an argument:\footnote{Program \boxml{kitdemo/fold3.sml}.}
\begin{verbatim}
       fun foldl' f b xs = 
         let fun loop(p as ([], b))= p
               | loop(x::xs, b) = loop(xs,f(x,b)))
...
A 
\index{project}\index{project file}%
...
Project files have file extension \index{{\tt .pm}@pm}{\tt .pm}. The 
...
\index{path!absolute}\index{path!relative}paths.  Relative paths are
...
\index{project file!comment in}%
\index{comment!in project file}%
comments. 
...
executed only the first time it is imported. 
...
files were
recompiled.\index{recompilation}\index{projects}
\index{source file}%
...
program from scratch.  As an example, consider the project file (\boxml{kitdemo/scan.pm}) for the text
...
linked, it can be executed by running the command\index{run@{\tt run}}
\begin{verbatim}
 run
\end{verbatim}
...
seeks to {\em match\/}\index{matching} the new exported information to
...
example. Consider the project {\tt set.pm},\footnote{Project
  \boxml{kitdemo/set.pm}.} which mentions the source files {\tt poly\_set.sml} and {\tt int\_set.sml}. The source
...
the {\tt Set}~functor in the source file \boxml{SetApp.sml} into
separate code:
\begin{verbatim}
...
\index{scan_rev1.pm@{\tt scan\_rev1.pm}}%
...
\index{Profiling@{\tt Profiling} sub-menu}%
{\tt Profiling} sub-menu:\footnote{The 
  \index{instruction count profiling@\texttt{instruction count profiling}}%
...
\index{region profiling@{\tt region profiling}}%
\mbox{{\tt region profiling}}. If you want the Kit to report
region-annotated programs with program points, you should enable
either 
\index{print physical size inference expression@{\tt print physical size inference expression}}%
...
\index{print program points@{\tt print program points}}%
{\tt print program points} from the {\tt Layout} sub-menu.

To make the compiler report a region flow graph, enable
\index{show region flow graph and generate .vcg file@{\tt show region flow graph and generate .vcg file}}%
...
contains the following fragment:\label{reg_flow_graph.ex}
\begin{verbatim}
 cp_list[r314:inf]
    --r314 sat-->   [*r314*] ;
...
\texttt{cp\_list[r314:inf]} means that \texttt{r314} is a formal
...
\begin{verbatim}
 cp_list[r314:inf] --r314 sat--> [*r314*] ;
\end{verbatim}
is read: there is an edge from node \texttt{r314} to node
\texttt{r314} and node \texttt{r314} has been written earlier. From
...
% I don't understand the scenario below; can it happen? (ME 1998-09-27)
%
%\noindent
...
\end{verbatim}
Here, we have a scc node (id 206) containing region variable
\texttt{r1181} and an edge to scc node (id 205) containing region
variable \texttt{r1486}.
...
the menu item
\index{paths between two nodes in region flow graph@{\tt paths between two nodes in region flow graph}}%
...
search for \texttt{Starting layout of paths}:\footnote{Because region
  ...
  the project again to make sure that the regions you have specified
...
started by typing \texttt{xvcg} at the command prompt.  We use file
\texttt{life.vcg} as the running example. Typing \mbox{\texttt{xvcg
...
\caption{After choosing the
  ...
  \texttt{r1748}. If you have generated the path from Section~\ref{regFlowPath.sec}, the option
  \texttt{Path2(r314,r1588)} is present.}
\label{vcg3.fig}
\end{figure}
...
\caption{The figure shows the path between
  node \texttt{r474} and \texttt{r1748}. If you look on
  page~\pageref{reg_flow_graph.ex}, you may notice that it is the same
  path as in the log file; the numbers have changed however, because
  they where generated in two different compilations.}
\label{vcg4.fig}
\end{figure}
...
\end{verbatim}
You can change the 
\index{profile strategy!option@options}%
...
\begin{description}
\item[{\tt -realtime}:\index{realtime@\texttt{-realtime} option}\index{timer!real}] 
  Real time.
\item[{\tt -virtualtime}:\index{virtualtime@\texttt{-virtualtime} option}\index{timer!virtual}] 
  The execution time for the process.
\item[{\tt -profiletime}:\index{profiletime@\texttt{-profiletime} option}\index{timer!prof}] 
  The execution time for the process together with the time used in the operating system on behalf of the process.
\end{description}
...
There are several other command line arguments; these options can be
seen by using the \texttt{-h} or 
\index{help@{\tt -help} option}%
\texttt{-help} options.
...
regions and it does not depend on the specifics of the runtime
...
infinite region operations such as {\tt allocateRegionInf} and {\tt
  alloc}. The program has allocated 95764 infinite regions and
deallocated 95761; the three global regions are not deallocated
before the region statistics is printed and the program terminates.
The program has allocated 858873 objects in infinite regions. It has
...
program had allocated the largest amount of memory in infinite
regions.

In the {\tt STACK} part, we see that the program has allocated and
deallocated the same number of finite regions. We also see that the
...
searching for program point \texttt{pp64} in the region-annotated
lambda program.
...
leave the Kit, type \index{quit@{\tt quit}}\boxml{quit} followed by a
return character. 
...
%------------------------------------------------
The menu 
\index{Printing of intermediate forms@{\tt Printing of intermediate forms}}%
...
\index{statistics after optimisation@{\tt statistics after optimisation}}%
...
\index{print atbot expression@{\tt print atbot expression}}%
...
\index{print drop regions expression@{\tt print drop regions expression}}%
...
\index{print KAM code before register allocation@{\tt print KAM code before register allocation}}%
\texttt{print KAM code before register allocation}. The result of
\index{register allocation}%
register allocation can be viewed by enabling the flag 
\index{print KAM code after register allocation@{\tt print KAM code after register allocation}}%
...
The flags 
\index{print types@{\tt print types}}%
\texttt{print types},
\index{print effects@{\tt print effects}}%
\texttt{print effects}, and
\index{print regions@{\tt print regions}}%
...
Enabling the flag
\index{print in K-Normal Form@{\tt print in K-Normal Form}}%
\texttt{print in K-Normal Form}\index{K-normalisation} causes
...
called \indexTwo{prim}{\texttt{prim}}, which is declared to have type
...
The first string \boxml{"concatString"} denotes a C
function identifier.\footnote{Some strings (e.g., \boxml{"="} and \boxml{":="}) are
  recognised and implemented in assembler by the compiler.} For the
example declaration, the Kit generates a call to the C function \boxml{concatString} with arguments {\tt s} and {\tt s'}. The C
...
function \boxml{raiseif0} may be declared thus:
\begin{verbatim}
...
passed as the first and second argument to \boxml{prim}.
...
\begin{example}\label{power_exn.ex}
  The \indexTwo{power_exn}{\texttt{power\_exn}} function shows how an
...
\begin{example}\label{dir.ex}
  The \indexTwo{dir}{\texttt{dir}} function shows how a list can be
...
\begin{example}\label{real_list.ex}
  Function \indexTwo{real_list}{\texttt{real\_list}} constructs a list
