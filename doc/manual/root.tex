...
useful operations on characters and strings.\footnote{Consult the
  files {\tt basislib/CHAR.sml}, {\tt basislib/STRING.sml},
...
applicationn of \boxml{sieve} ensures that the list bound to {\tt
...
applying the map $\{\epsilon\mapsto\{\Get(\rho_0),\Put(\rho_2)\}\}$ to
the effect $\{\Get(\rho_9),\epsilon\}$, say, would presumably yield
...
Compiling a project is easy; simply select the entry {\tt Compile file
...
memorises that if you refer to {\tt IntSet1.mem}, for instance, then
it is actually {\tt PolySet.mem} that is applied with type instance
{\tt int}.
...
{\tt ('a -> string) -> 'a list -> string}. The source file {\tt
  Set.sml} is listed in Figure~\ref{Set.fig}. It declares the functor
{\tt Set}, which takes as arguments the element type for the set, an
ordering function on elements, and a function for providing a string
...
\nocite{total97,total94,btv96,elshal95,KochHojfeld96,H96,hallenberg99}
