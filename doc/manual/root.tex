...
paired with a region variable.\index{boxing}\index{value!boxed}\index{value!unboxed}
...
Here is code, which
uses these ideas:\index{cp@{\tt cp}}\index{msort@{\tt msort}}\index{merge sort}\footnote{Project {\tt kitdemo/msort.pm}, file {\tt kitdemo/msort.sml}. To learn how to compile projects, see Chapter~\ref{modules_and_projects.chap}.}
\pagebreak
\begin{verbatim}
       fun cp [] =[]
         | cp (x::xs)= x :: cp xs
...
this evaluation yields a value $v$. Here $v$ may be a \index{boxing}boxed 
or an unboxed value.
...
ref & \forall\alpha\rho_2\epsilon.\alpha \ar{\epsilon.\{\Put(\rho_2)\}}(\alpha\,\REF,\rho_2)\cr
!   &  \forall\alpha\rho_2\epsilon.(\alpha\,\REF,\rho_2)\ar{\epsilon.\{\Get(\rho_2)\}} \alpha \cr
:=  & \forall\alpha\rho_2\rho_3\epsilon.((\alpha\,\REF,\rho_2)\ast\alpha,\rho_3)
