...
\boxml{(a,b)} 
\index{arguments!unboxed}%
\index{function arguments!unboxed}%
...
%mael - do the testing when the profiler becomes available
...
list.  The space behavior of the five ways of computing the length
vary considerably. We shall have more to say about the time behavior
in what follows.
...
the Kit) when the function returns. Thus, to obtain the same behavior
...
\begin{figure}
\begin{center}
  \scalebox{0.5}{\includegraphics{bootstrap.eps}}
\end{center}
...
is pictured to the right of the SML/NJ executable and is created using
the standard built procedure:
\begin{verbatim}
[23:01-kit]# pwd
/import/home/nh/ITU/MLKit/mlkit/kit
[23:02-kit]# make clean
...
[23:04-kit]# make
...
[23:11-kit]# make install INSTDIR=/import/home/nh/ITU/MLKit/mlkit-v1
...
[23:14-kit]# make bootstrap INSTDIR=/import/home/nh/ITU/MLKit/mlkit-v1
...
[23:16-kit]# cd /import/home/nh/ITU/MLKit/mlkit-v1
...
[23:25-mlkit-v1]# make bootstrap INSTDIR=/import/home/nh/MLKit/mlkit-v2
\end{verbatim}

To construct a version of the ML Kit that is based on the ML Kit runtime
system, execute the command
\begin{verbatim}
  > $(HOME)/mlkit-v1/bin/mlkit -gc sources.pm
  ...
\end{verbatim}
Assuming everything works out fine, an executable version of the ML
Kit is now available as the file {\tt run} in the current working
directory; this version of the ML Kit executable is based on the ML
Kit runtime system.\footnote{Unfortunately, because of the engineering
  of the recompilation scheme used in the ML Kit, it is not at present
  possible to memorize the compilation of the Standard ML Basis
  Library when using the bootstrapped version of the compiler.}

It is left as an exercise to the reader to verify that after two more
iterations of bootstrapping, the resulting ML Kit is identical to the
version of the ML Kit used for compilation; the reason that two
iterations are necessary has to do with the way that machine code
labels are choosen when memorization of the Basic Library is supported
(compared to when memorization is not supported.)

\subsection{Testing while bootstrapping}

You can easily apply all the ML Kit tests in the test directory. This
will, perform an acceptance test on the four configurations of the ML:
...
Below we show the process of building the kits 
\end{enumerate}

\begin{verbatim}
[23:38-kit]# pwd
/home/nh/ITU/MLKit/mlkit/kit
[23:40-kit]# make clean
...
...
\index{function argument!unboxed}%
...
\begin{verbatim}
ML Kit version 3.4.0, May 14, 2001 [X86 Backend]
