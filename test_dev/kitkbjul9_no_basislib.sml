...
(* Lars *)
fun print_string (x:string):unit = print x
  ...
  | pretty_term (Term (oper,sons)) =
    if mem eq_string oper INFIXES then
      case sons of
        ...
        | _ => raise FailPretty (* "pretty_term : infix arity <> 2"*)
    else
      (print_string oper;
       case sons of
	   []   => ()
...
fun eq_ordering (Greater,Greater) = true
...
fun rem_eq equiv =
    let fun remrec x [] = raise FailRemEQ
...
fun copy_arg(interm:bool, n, rules, failures, p, eps) =
    (interm, n+0, copy_rules rules, copy_termpairlist failures, copy_int_pair p, copy_termpairlist eps)
