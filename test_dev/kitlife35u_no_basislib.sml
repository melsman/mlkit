...
and mk_nextgen_fn gen =
    if true then
	  ...
	  val newnbrlist = copy_list(collect (fn z => filter (fn x => not( isalive x))
                                                             ( neighbours z)) living) (* unfolded o twice*)
...
local val xstart = 0 and ystart = 0
      fun markafter n string = (let val r = string ^ spaces n ^ "0" in r end)
      fun plotfrom (x,y) (* current position *)
                   str   (* current line being prepared -- a string *)
                   ((x1,y1)::more)  (* coordinates to be plotted *)
          = if eq_integer(x,x1)
            then (* same line so extend str and continue from y1+1 *) (plotfrom(x,y1+1)(markafter(y1-y)str)more)
...
in  fun plot coordlist = let


      val r1 = filter good coordlist

      val r2 = copy_list(r1)

      val r3 = plotfrom(xstart,ystart) "" (r2)

      val r = map_rec(copy_string,(r3))
    in r end
end
