#!/bin/sh
# Run from any directory. Requires an Apple Silicon Mac.
set -eu
cd "$(dirname "$0")/../../.."
[ "$(uname -s)" = Darwin ] && [ "$(uname -m)" = arm64 ] || {
  echo 'Run on an Apple Silicon Mac.' >&2; exit 1;
}
...
arch=arm64
run configure-default env -u DARWIN_NATIVE ./configure CC=gcc
