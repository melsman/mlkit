...
/* scan the CInterfaceRootList */
    { 
	ml_val_t	tmp;
	bibop_t		bibop = BIBOP;
	ml_val_t	w, *p;

	for (tmp = CInterfaceRootList; tmp != LIST_nil; tmp = LIST_tl(tmp)) {
	  /* this was the original code, which just walked the whole list */
	  /* MinorGC_CheckWord(bibop,gen1,(ml_val_t *) LIST_hd(tmp)); */
	    p = (ml_val_t *)LIST_hd(tmp);
	    w = *(p);
	    if (isBOXED(w)) {
		aid_t aid = ADDR_TO_PAGEID(bibop, w);
		if (aid==AID_NEW)
		    *(p) = MinorGC_ForwardObj(gen1, w);
		else
		  /* abort as soon as we are not in alloc space anymore */
		    break;
	    }
	    else
	      /* this is possible since the list only grows monotonically */
		break;
	}
    }
#endif
...
PVT ml_val_t MinorGC_ForwardObj (gen_t *gen1, ml_val_t v)
{
    ...
    ml_val_t	*obj_start, *new_obj, desc;
    Word_t	len;
    arena_t	*arena;
  ...
  /* Find the object header (object may be derived) */
	...
	new_obj = arena->nextw;
	arena->nextw += 3;
	*new_obj++ = MAKE_DESC(PAIR_SZW, DTAG_record);
#else
	arena = gen1->arena[PAIR_INDX];
	new_obj = arena->nextw;
	arena->nextw += 2;
#endif
	new_obj[0] = obj_start[0];
	...
	break;
      case DTAG_special:
	return MinorGC_FwdSpecial (gen1, obj_start, desc);
      case DTAG_string: {
	...
	break;
      default:
	Die ("bad object tag %d, obj_start = %#x, obj = %#x, desc = %#x",
	    GET_TAG(desc), obj_start, obj, desc);
    } /* end of switch */
		...
		*new_obj = v;
		...
		if (aid == AID_NEW) {
		    ...
		    if (desc == DESC_forwarded) {
		       ...
		       */
#ifdef DEBUG_WEAK_PTRS
SayDebug (" already forwarded to %#x\n", PTR_CtoML(FOLLOW_FWDOBJ(v_start, vp)));
#endif
			*new_obj++ = DESC_weak;
			*new_obj = v;
		    }
