    ...
    structure E = struct
      ...
      local
        ...
        local
          ...
          val refTy = mk_Arrow0 (alphaTy, Type.mk_Ref alphaTy)

          val beta = TyVar.fresh_normal ()
          val betaTy = Type.from_TyVar beta

          val refTy_to_TE = mk_Arrow0 (betaTy, Type.mk_Ref betaTy)
        ...
        local
          ...
          val ve =
              VE.singleton_con (Ident.id_INTINF,
                                TypeScheme.from_Type (mk_Arrow0 (recordTy, intinfTy)),
                                [Ident.id_INTINF])
        in
        ...
        local
          ...
          fun conVE id ty1 ty2 =
            VE.singleton_con (id,
                              TypeScheme.from_Type (mk_Arrow0 (ty1, fragTy ty2)),
        ...
        local
          ...
          val sigma_prim =
              let val alpha = TyVar.fresh_normal ()
                  ...
                  val beta = TyVar.fresh_normal ()
                  val betaTy = Type.from_TyVar beta
                  val tau_prim = mk_Arrow0 (Type.from_pair (Type.String, alphaTy), betaTy)
          ...
          (* \/'a,'b. string * ('a -> 'b) -> unit *)
          val sigma_export =
              let val exportArrowTy = mk_Arrow0
                      (Type.from_TyVar(TyVar.fresh_normal()),
                       Type.from_TyVar(TyVar.fresh_normal()))
                  val tau_export = mk_Arrow0
          ...
          val tau_num_X_num_to_num =
                mk_Arrow0 (Type.from_pair (tau_num, tau_num), tau_num)
          ...
          val tau_realint_to_realint = mk_Arrow0 (tau_realint, tau_realint)
          ...
          val tau_numtxt_X_numtxt_to_bool =
                mk_Arrow0 (Type.from_pair (tau_numtxt, tau_numtxt),
                           Type.Bool)
          ...
          val tau_wordint_X_wordint_to_wordint =
                mk_Arrow0 (Type.from_pair (tau_wordint, tau_wordint),
                           tau_wordint)
        ...
        local
          fun mk_sigma () = (*construct the type scheme: forall 'a.'a->unit*)
            let val _ = Level.push()
                val alpha = TyVar.fresh_normal ()
                val alphaTy = Type.from_TyVar alpha
                val arrowType = mk_Arrow0 (alphaTy, Type.Unit)
                val _ = Level.pop()
            in
