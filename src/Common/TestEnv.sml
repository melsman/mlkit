...
functor TestEnv(structure TestInfo: TEST_INFO
  ...
  struct
    ...
    fun new_version_dir () = OS.Path.concat(!test_env_directory, "Result")                (* Should _not_ be changed. *)
    val target_directory = new_version_dir                                                (* Should _not_ be changed. *)

    fun test_report_filename () = OS.Path.concat(new_version_dir(), "test_report")        (* Should _not_ be changed. *)
    fun test_report_dvi () = OS.Path.concat(new_version_dir(), "test_report.dvi")         (* Should _not_ be changed. *)
	...
	val shell_command =
	  case arch_os()
		...
		(memtime_exe ^ " -f " ^ outputfile ^ " -o " ^ tempfile ^ " -t " ^ temp ^ " " ^ program)
	     ...
	     | (arch,os) => raise Crash_test ("Error in memtime, " ^ arch ^ "-" ^ os ^ " not supported.")
...
*)
	...
	in
	  ...
	  ["\\documentclass[a4paper,10pt,oneside]{article}",
	   ...
	   "\\oddsidemargin -8.4mm",
	   "\\title{ML Kit Test Report \\\\ for \\\\ the ML Kit with Regions}",
	   "\\author{Author: The ML Kit Test Environment}", 
	   "\\date{\\today}",
	   "\\begin{document}",
	   "\\maketitle",
	   "\\abstract{This test report is generated by the test environment in the ML Kit. ",
	   "All tests has been executed on {\\bf " ^ (get_env_var "HOST") ^ "}. ",
	   "The version of the compiler that has been used runs under and generates code for the {\\bf ",
	   arch ^ "-" ^ os ^ "} system, using the {\\bf " ^ backend ^ "} backend of the ML Kit.}",
	   "\\tableofcontents"]
	end
...
(*	       val log_filename = new_compile_strategy_dir ^ filename ^ ".log" *)
		 ...
		 in
	       ...
	       val size_of_stripped_exec = (execute_shell_command ("gstrip " ^ exe_file);
