    ...
    fun build {flags, mlbfile, target} : unit =
        let 
          ...
          val srcs_scriptsonly =
              MlbProject.fold 
                  (fn (MlbProject.Script sml,_,_,acc) => (MlbProject.Atom.toString sml) :: acc
                    | (MlbProject.NonScript sml,_,_,acc) => acc) [] srcs_mlbs_anns_all
              
          val srcs_allbutscripts = 
              MlbProject.fold 
                  (fn (MlbProject.NonScript sml,_,_,acc) => (MlbProject.Atom.toString sml) :: acc
...
*)                
          ...
          type Acc = {regionVar:unit -> int,
                      ...
                      children:Running.R, 
                      bg:MlbProject.BG, 
                      queue : ((MlbProject.File * string * string list) Queue.Q)}
          ...
          val left = run {regionVar = fn () => initialRev, unique = 1, children = Running.empty, bg = srcs_mlbs_anns_all,
                          queue = List.foldl enqueue
                                             (Queue.empty (fn ((a,_,_),(b,_,_)) => 
                                                              (MlbProject.Atom.compare (strip (MlbProject.project a), 
                                                                                        strip (MlbProject.project b))))) first}
