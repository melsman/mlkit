...
struct
  ...
  fun staticData () = List.concat (rev (!staticChunks))
  ...
  fun number n dst = numberInto n dst []
  ...
  fun read fsz aty dst = readInto fsz aty dst []
  ...
  fun write fsz dst src = writeInto fsz dst src []
  ...
  fun arguments fsz args = argumentsInto fsz args []
  ...
  fun results fsz res = resultsInto fsz res []
  ...
  fun mlcall tail fsz target call = mlcallInto tail fsz target call []
  ...
  fun internalCall fsz name args = internalCallInto fsz name args []
  ...
  fun foreignCallInto fsz name args convert code =
    if name=":" then unsupported "dynamic foreign symbol resolution"
      ...
      {name=name,fixed=map (fn _ => AbiArm64.I64) args,variadic=[],protectGC=gc(),
       loadArgument=fn (i,extra) => readInto (fsz+extra) (List.nth(args,i)) (X 16)
                                    (convert(i,X 16))} code
  fun foreignCall fsz name args convert = foreignCallInto fsz name args convert []
  fun autoCall fsz {name,args:(SS.Aty*LS.foreign_type) list,rhos_for_result,res=(dst,ft)} =
    let
      fun convert (i,r) = case #2(List.nth(args,i)) of
    ...
    in
      save @ foreignCall (fsz+(if boxed then 2 else 0)) name (map #1 args) convert @ result @ write fsz dst (X 0)
    end
  ...
  fun allocate fsz sma words =
    ...
    in
      internalCall fsz "mlkit_arm64_alloc" [a,integer words,integer mode,integer 0,integer(programPoint sma)]
    end
  ...
  fun recordWithUntagInto untag fsz pat alloc prefix elems code =
    let
      ...
      val code = foldri (fn (i,fragment,code) =>
        fragment @ loadInto (SP,0,X 17) (storeInto (X 16,X 17,8*i) code))
  ...
  fun recordWithUntag untag fsz pat alloc prefix elems =
    recordWithUntagInto untag fsz pat alloc prefix elems []
  ...
  fun primitive fsz {name,args,res} =
    let
      ...
      fun checked opn = case (args,res) of
          ...
          let
            val ok=localFresh()
          in
            read fsz a (X 16) @ read fsz b (X 17) @ [ins opn ["x16","x16","x17"],
              ins "b.vc" [pr_lab ok]] @ overflow() @ [Label ok] @ write fsz d (X 16)
          end
      ...
      fun failUnless cc =
        let
          val ok=localFresh()
        in
          [ins ("b."^cc) [pr_lab ok]] @ overflow() @ [Label ok]
        end
      ...
      fun shift opn (rep as (bits,_,box,_)) =
          let
            ...
            val done=localFresh()
          in
            getnum rep a (X 16) @ stack(true,16) @ store(X 16,SP,0) @
             ...
             [Label done] @ putnum rep buffer d
          end
      ...
      fun tableSub scale (rep as (_,_,box,_)) =
          ...
          in
            index t i scale @ [ins "ldr" [if scale=2 then "w16" else "x16","[x17, #8]"]] @
             putnum rep buffer d
          end
    ...
    | Fresh_exname =>
        ...
        in
          [Label retry,ins "ldaxr" ["x16","[x17]"],ins "add" ["x16","x16","#1"],
              ins "stlxr" ["w30","x16","[x17]"],ins "cbnz" ["w30",pr_lab retry]]
        end
  ...
  and stmtInto fsz ls code =
    ...
    | LS.ASSIGN {pat,bind=LS.STORE(aty,l)} =>
        ((dataLabel l; read fsz aty (X 16) @ address(DatLab l,X 17) @ store(X 16,X 17,0) @
         constant(1,X 16) @ write fsz pat (X 16))) @ code
    ...
    | LS.ASSIGN {pat,bind=LS.BLOCKF64{elems,alloc,tag}} =>
        recordInto fsz pat alloc [constant(IntInf.fromInt(Word.toInt tag),X 16)] elems code
    | LS.ASSIGN {pat,bind=LS.SCRATCHMEM{bytes,alloc,tag}} =>
        (if bytes=0 then constant(1,X 16) @ write fsz pat (X 16)
          ...
          constant(IntInf.fromInt(Word.toInt tag),X 17) @ store(X 17,X 16,0) @ write fsz pat (X 16)) @ code
    ...
    | LS.ASSIGN {pat,bind=LS.CLOS_RECORD{label,elems=elems as (_,_,rhos),alloc,f64_vars}} =>
        recordInto fsz pat alloc (header(BackendInfo.tag_clos(false,1+length(LS.smash_free elems),1+length rhos+f64_vars)) @ [address(MLFunLab label,X 16)]) (LS.smash_free elems) code
    ...
    | LS.ASSIGN {pat,bind=LS.PASS_PTR_TO_MEM(alloc,n,untag)} =>
        ...
        in
          internalCall fsz "mlkit_arm64_alloc" [a,integer(n-1),integer mode,integer 1,integer(programPoint alloc)]
        end
         else allocate fsz alloc n)) @ writeInto fsz pat (X 16) code
    | LS.ASSIGN {pat,bind=LS.PASS_PTR_TO_RHO{sma}} =>
        (
        let
          val (a,mode)=regionArg sma
        in
          read fsz a (X 16) @ (case mode of 0 => [ins "and" ["x16","x16","#-3"]]
            | 2 => [ins "orr" ["x16","x16","#2"]] | _ => []) @ write fsz pat (X 16)
        end) @ code
    | LS.ASSIGN {pat,bind=LS.CON0{con,con_kind,aux_regions,alloc}} =>
        (
        let
          val reset=stmts fsz [LS.RESET_REGIONS{force=false,regions_for_resetting=aux_regions}]
        ...
        | LS.BOXED i => reset @ record fsz pat alloc [constant(IntInf.fromInt(Word.toInt(BackendInfo.tag_con0(false,i))),X 16)] []
        end) @ code
    | LS.ASSIGN {pat,bind=LS.CON1{con_kind,alloc,arg,...}} =>
        ((case con_kind of LS.BOXED i => record fsz pat alloc
         ...
         | LS.UNBOXED_HIGH i => read fsz arg (X 16) @ constant(IntInf.fromInt i*281474976710656,X 17) @
           [ins "orr" ["x16","x16","x17"]] @ write fsz pat (X 16)
         | _ => unsupported "unary enumeration")) @ code
    | LS.ASSIGN {pat,bind=LS.DECON{con_kind,con_aty,...}} =>
        readInto fsz con_aty (X 16) (((case con_kind of LS.BOXED _ => load(X 16,8,X 16)
          | LS.UNBOXED 0 => []
          | LS.UNBOXED _ => [ins "and" ["x16","x16","#-4"]]
          | LS.UNBOXED_HIGH _ => [ins "and" ["x16","x16","#0xffffffffffff"]]
          | _ => unsupported "enumeration deconstruction")) @ writeInto fsz pat (X 16) code)
    ...
    | LS.CCALL {name="spawnone",args=[arg],rhos_for_result=[],res=[res]} =>
        ...
        let
          ...
          val ()=addStatic ([Directive ".text",Directive ".p2align 2",Label entry] @
    ...
    | LS.EXPORT{name,clos_lab,arg=(aty,ft1,ft2)} =>
        ...
        let
          ...
          val ()=addStatic ([Directive ".data",Directive ".p2align 3",Label ctx,Directive ".quad 0"] @
        ...
        in
          read fsz aty (X 16) @ address(DatLab clos_lab,X 17) @ store(X 16,X 17,0) @
           address(ctx,X 17) @ store(X 28,X 17,0) @
           address(str,X 16) @ address(NameLab name,X 17) @
           internalCall fsz "sml_regCfuns" [SS.PHREG_ATY(X 16),SS.PHREG_ATY(X 17)]
        end) @ code
    ...
    | _ =>
        unsupported (LS.pr_line_stmt SS.pr_sty SS.pr_offset SS.pr_aty true ls)
  ...
  and stmts fsz statements = stmtsInto fsz statements []
  ...
  fun CG {main_lab,code,imports,exports,safe} =
    let
      ...
      fun marker suffix =
            ...
            in
              [Directive ".data",Directive ".p2align 3",Directive(".globl " ^ pr_lab l),Label l]
            end
    ...
    in
      text @ marker "begin" @ List.concat(map data (!dataLabels)) @ staticData () @ marker "end" @
       (if gc() then metadata else [])
    end
  ...
  fun linkCode repl (labs,_) =
    let
      ...
      fun exnData(i,(name,display,lab)) =
        ...
        in
          datum l (if tagged() then ["0x" ^ Word.toString(BackendInfo.tag_exname true),pr_lab l ^ "+16",
          "0x" ^ Word.toString(BackendInfo.tag_excon0 true),Int.toString i,pr_lab str]
          else [pr_lab l ^ "+8",Int.toString i,pr_lab str]) @ datum (DatLab lab) [pr_lab l]
        end
    ...
    in
      function(NameLab "code") @ move(X 0,X 28) @
  ...
  fun generate_repl_link_code (name,labs) =
    let
      ...
      val metadata=if not(gc()) then [] else
          ...
          [ins "bl" ["_mlkit_arm64_register_image"]]
    in
      function(NameLab name) @ saveC() @ address(NameLab "top_ctx",X 16) @ load(X 16,0,X 28) @ metadata @
