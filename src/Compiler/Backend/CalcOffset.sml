...
(* Memo:
  ...
  local
    ...
    fun lvset_difference (lv_set,lv_list,LVmap) =
        foldr (fn (offset,set) => IntSet.remove offset set) lv_set (lookup_lvs(LVmap,lv_list))

    fun lvset_add (lv_set,lv_list,LVmap) =
        foldr (fn (offset,set) => IntSet.insert offset set) lv_set (lookup_lvs(LVmap,lv_list))
	...
	fun set_bit (bit_no,w) =
            Word32.orb(w,Word32.<<(Word32.fromInt 1,Word.fromInt bit_no))

	val offsets_in_bitvector_sorted =
            ListSort.sort (fn i1 => fn i2 => i1 < i2) (IntSet.list L_set)
	...
	val num_words =
	    if size_fd mod 32 = 0 then size_fd div 32
	    else (size_fd div 32)+1
