...
struct
  ...
  in fun add_dynamic (name,l1,l2) = dynamic := Binarymap.insert(!dynamic, name, (l1,l2))
    ...
    fun offset_bytes (WORDS w) = i2s (4*w)
...
*)
    ...
    fun gen_data_lab lab = add_static_data [I.dot_data,
                                            I.dot_align 4,
                                            I.lab (DatLab lab),
                                            I.dot_long (i2s BI.ml_unit)]  (* was "0" but use ml_unit instead for GC 2001-01-09, Niels *)
      ...
      fun callc_static_or_dynamic (name : string, nargs, fnlab, C) = 
          case name of 
              ...
              let 
                  val () =
		      if nargs < 1 then
			  die "callc_static_or_dynamic: Dynamic liking requires a string as first argument."
		      else ()
    ...
    (* push_args: general function for pushing arguments. 
    ...
    fun push_args push_arg size_ff tmp (args,C) =
        let fun loop ([], _) = C
    ...
    fun pop_args name nargs C = 
        case nargs
         of 0 => C
          | n => I.addl(I (i2s (4* (case name of ":" => n-1 | _ => n))), R esp) :: C
	...
	in
	    ...
	    I.pushl(R tmp) ::
	    iterl (fn (i,C) =>
		     I.movl(D(i2s(~4*i), tmp), R tmp1) ::  (* notice: for x86, esp points to the last slot used *)
		     I.pushl(R tmp1) :: C	     
		 )
		 C nargs
	end
	...
	let val tmp = tmp_reg0
	in I.movl(D(i2s(4*nargs), esp), R tmp) ::  (* notice: for x86, esp points to the last slot used *)
	   I.movl(R tmp, R esp) :: 
	   C
	end

    fun callc push_arg size_ff dynlinklab tmp name args C =	
	let val nargs = List.length args
	in push_args push_arg size_ff tmp
		     (args,
		      if needs_align name then
			  align (nargs, 
				 callc_static_or_dynamic (name, nargs, NameLab dynlinklab,
							  restore_stack_alignment (nargs, C)))
		      else 
			  callc_static_or_dynamic (name, nargs, NameLab dynlinklab,
						   pop_args name nargs C)
		     )
	end
    ...
    fun compile_c_call_prim (name:string, args:SS.Aty list, opt_ret:SS.Aty option, size_ff:int, tmp:reg, C) =
      let
        ...
        fun store_ret(SOME d,C) = move_reg_into_aty(eax,d,size_ff,C)
          | store_ret(NONE,C) = C
      in callc push_arg size_ff "localResolveLibFnManual" tmp name args (
	 store_ret(opt_ret,C))
      end
    ...
    fun compile_c_call_auto (name,args,opt_res,size_ff,tmp,C) =
      let
        ...
        fun push_foreignptr (aty,size_ff,C) =
          if BI.tag_values() then 
            case aty of
		SS.PHREG_ATY r => (I.leal(D("-1", r), R tmp) :: 
                                   I.pushl(R tmp) :: C)
        ...
        fun push_chararray (aty,size_ff,C) = 
          case aty of
	      SS.PHREG_ATY r => (I.leal(D("4", r), R tmp) :: 
                                 I.pushl(R tmp) :: C)              
            | _ => move_aty_into_reg(aty,tmp,size_ff,
      ...
      in callc push_arg size_ff "localResolveLibFnAuto" tmp name args (
...
*)
  ...
  I.pushl(I (i2s size_ccf)) ::
  I.pushl(I (i2s size_rcf)) ::
  I.pushl(I (i2s size_spilled_region_args)) ::
          I.jmp(L gc_stub_lab) ::
          I.lab l :: C)
        end
      else C
      ...
      let val n = if region_profiling() then n0 + BI.objectDescSizeP 
          ...
          fun post_prof C =
            if region_profiling() then   (* tmp_reg1 now points at the object descriptor; initialize it *)
              I.movl(I (i2s pp), D("0",tmp_reg1)) ::               (* first word is pp *)
              I.movl(I (i2s n0), D("4",tmp_reg1)) ::               (* second word is object size *)
              I.leal(D (i2s (4*BI.objectDescSizeP), tmp_reg1), R tmp_reg1) :: C  (* make tmp_reg1 point at object *)
            else C
      in 
        copy(t,tmp_reg1,
        I.pushl(LA l) ::
      ...
      let val n0 = size_alloc (* size of untagged pair, e.g. *)
          ...
          fun post (t, C) =
            if region_profiling() then   (* tmp_reg1 now points at the object descriptor; initialize it *)
              I.movl(I (i2s pp), D("0",tmp_reg1)) ::               (* first word is pp *)
              I.movl(I (i2s n0), D("4",tmp_reg1)) ::               (* second word is object size *)
              I.leal(D (i2s (4*(BI.objectDescSizeP-1)), tmp_reg1), R t) :: C  (* make tmp_reg1 point at 
                                                                                         * word before object *)
            else 
    ...
    fun store_pp_prof (obj_ptr:reg, pp:LS.pp, C) =
      if region_profiling() then 
        ...
        else I.movl(I(i2s pp), D("-8", obj_ptr)) :: C
      else C
    ...
    fun force_reset_aux_region_kill_tmp0(sma,t:reg,size_ff,C) = 
      ...
      fun cmpi_kill_tmp01 {box} (jump,x,y,d,size_ff,C) = 
        ...
        in
           ...
           compare (
           jump true_lab ::
           I.movl(I (i2s BI.ml_false), R d_reg) ::
           I.jmp(L cont_lab) ::         
           I.lab true_lab ::
           I.movl(I (i2s BI.ml_true), R d_reg) ::
           I.lab cont_lab :: C')))
        end
...
*)        I.leal(DD("1", d_reg, d_reg, ""), R d_reg) ::
     ...
     fun cmpf_kill_tmp01 (cond,x,y,d,size_ff,C) =
       ...
       in
         ...
         jump true_lab ::
         I.movl(I (i2s mlFalse), R d_reg) ::
         I.jmp(L cont_lab) ::         
         I.lab true_lab ::
         I.movl(I (i2s mlTrue), R d_reg) ::
         I.lab cont_lab :: 
         C')
       end
                     ...
                     let val offset = if BI.tag_values() then 1 else 0
                         ...
                         fun maybe_tag_value C =
                           ...
                           if BI.tag_values() andalso tag_pairs_p() then  
                             I.movl(I (i2s(Word32.toInt(BI.tag_ref(false)))), 
...
*)
        ...
        fun store_exported_data_for_gc (labs,C) =
          if gc_p() then
            ...
            (I.pushl (I (i2s (List.length labs))) ::
        ...
        val static_data = 
          slots_for_datlabs(global_region_labs,
                            ...
                            I.dot_long (i2s initial_exnname_counter) ::
        ...
        fun allocate_global_regions(region_labs,C) = 
          let 
            fun maybe_push_region_id (region_id,C) = 
              if region_profiling() then I.pushl(I (i2s region_id)) :: C
              else C
...
*)
	    val nargs = if region_profiling() then 2 else 1
          in
            foldl (fn ((rho,lab),C) =>
                   let val region_id = Effect.key_of_eps_or_rho rho
		       val name = c_name rho
		       val C = I.movl(R eax, L (DatLab lab)) :: C
                   in
                       I.subl(I(i2s(4*BI.size_of_reg_desc())), R esp) ::
