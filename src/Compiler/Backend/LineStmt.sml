...
struct
  ...
  fun is_f64_atom at =
      case at of
          VAR lv => Lvars.get_ubf64 lv
        | FLOW_VAR (lv,_,_) => false
        | RVAR _ => false
      ...
      and layout_ls pr_sty pr_offset pr_aty simplify ls =
        ...
        in
          (case ls of
           ...
           | JMP{opr,args,reg_args,fargs,clos,res,bv} =>
               let
                 val t0 = HNODE{start="<",finish=">",childsep=RIGHT ",",children= map (layout_aty pr_aty) res}
                 val t1 = HNODE{start="<",finish=">",childsep=RIGHT ",",children=map (layout_aty pr_aty) args}
                 val t2 = HNODE{start="<",finish=">",childsep=RIGHT ",",children=[layout_aty_opt pr_aty clos]}
                 val t4 = HNODE{start="<",finish=">",childsep=RIGHT ",",children=map (layout_aty pr_aty) reg_args}
                 val t5 = HNODE{start="<",finish=">",childsep=RIGHT ",",children=map (layout_aty pr_aty) fargs}
               in
                 HNODE{start=flatten1(t0) ^ " = " ^ Labels.pr_label opr ^ "_funjmp ",
                       finish="", childsep=RIGHT " ",
                       children=[t1,t4,t5,t2]}
               end
           | FUNCALL{opr,args,reg_args,fargs,clos,res,bv=[]} =>
               let
                 val t0 = HNODE{start="<",finish=">",childsep=RIGHT ",",children= map (layout_aty pr_aty) res}
                 val t1 = HNODE{start="<",finish=">",childsep=RIGHT ",",children=map (layout_aty pr_aty) args}
                 val t2 = HNODE{start="<",finish=">",childsep=RIGHT ",",children=[layout_aty_opt pr_aty clos]}
        ...
        of ClosExp.VAR lv => maybe_assign (lvars_res, ATOM {aty=VAR lv}, acc)
         ...
         | ClosExp.FNCALL{opr,args,clos} =>
          FNCALL{opr=ce_to_atom opr,args=ces_to_atoms args,
                 clos=ce_to_atom_opt clos,res=map VAR lvars_res, bv=[]}::acc
         | ClosExp.JMP{opr,args,reg_args,clos,...} =>
           let val args = ces_to_atoms args
               val (fargs,args) = List.partition is_f64_atom args
           in JMP{opr=opr,args=args,reg_args=ces_to_atoms reg_args,fargs=fargs,
                  clos=ce_to_atom_opt clos,res=map VAR lvars_res,bv=[]}::acc
           end
         | ClosExp.FUNCALL{opr,args,reg_args,clos,...} =>
           let val args = ces_to_atoms args
               val (fargs,args) = List.partition is_f64_atom args
           in FUNCALL{opr=opr,args=args,reg_args=ces_to_atoms reg_args,fargs=fargs,
                      clos=ce_to_atom_opt clos,res=map VAR lvars_res,bv=[]}::acc
           end
  ...
  fun def_var_on_fun {opr,args,reg_args,fargs,clos,res,bv} = get_var_atoms(res,[])
  ...
  local
    ...
    fun use_var_on_fun' ignore_rvars {opr,args,reg_args,fargs,clos,res,bv} = (* Operand is always a label *)
      if ignore_rvars then
        get_var_atoms(args,get_var_atom_opt(clos,[]))  (* also ignore fargs *)
      else
        get_var_atoms(args,get_var_atoms(reg_args,get_var_atom_opt(clos,get_var_atoms(fargs,[]))))

    fun use_var_ls' ignore_rvars ls =   (* ignore_rvars_and_f64vars *)
      let
  ...
  in
    ...
    fun def_use_var_ls_cbv arg = (def_var_ls arg, use_var_ls' true arg)  (*calc bit-vectors *)
  end
  ...
  local
    fun map_lss f_aty f_offset f_sty lss =
      let
        ...
        fun map_fun_app {opr,args,clos,fargs,res,reg_args,bv} =
          {opr=opr,
           args=map_atys args,
           clos=map_aty_opt clos,
           reg_args=map_atys reg_args,
           fargs=map_atys fargs,
           res=map_atys res,
           bv=bv}
