...
(*	  val B = Eff.restrain B *)
  ...
  *)
  ...
  let
    ...
    fun maybe_explicit_rho (rse:rse) (B:cone) (rt:Eff.runType) (rv_opt:RegVar.regvar option) : place * cone =
        case rv_opt of
...
*)
    ...
    | E.PRIM(E.RESET_REGIONSprim{instance = _}, [e0 as (E.VAR _)] ) =>
	    ...
	    val mu = (R.unitType, fresh_rho)
            val phi = Eff.mkUnion(map Eff.mkPut(fresh_rho::List.filter Eff.is_rho (R.ann_mus mus0 [])))
          in
            case e' of
