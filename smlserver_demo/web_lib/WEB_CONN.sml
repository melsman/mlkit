...
(*
 ...
 [return s] sends HTML string s with status code 200 to client,
 including HTTP headers. May raise MissingConnection.

 [returnFile (sc,mt,f)] sends file f with status code sc to client,
 including HTTP headers. The mime type is mt. Raises MissingConnection
 if the execution is not associated with a connection. Raises
 Fail(msg) if the file cannot be opened for reading.

 [write s] sends string s to client, excluding HTTP headers. May raise
 MissingConnection.

 [returnRedirect loc] sends redirection HTTP response to client
 ...
 associated with the connection. It reads the POST content or the
 query string. The POST content takes precedence over the query
 string.

 [formvar k] returns the first query data associated with the key k;
 the function returns NONE if no query data is present for the
 argument key k.
 ...
 [headers()] returns, as a set, the HTTP headers associated with the
 connection.
 ...
 there is always only on location associated with a connection..

 [peer()] returns the name of the peer associated with the
 connection. The peer address is determined by the communications
 driver in use by the connection. Typically, it is a dotted IP
 address, for example, 199.221.53.205, but this is not guaranteed.
 ...
 makes it appear that the user requested a different URL and then run
 that request. This form of redirect does not require the running of
 an additional thread.

 [server()] returns the name of the server associated with the
 connection.

 [url()] return the url (relative to server-root) associated with the
 request.

 [hasConnection()] returns true if a connection is available. Returns
 false otherwise. For the execution of init scripts and scheduled
