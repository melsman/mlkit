(* File array2.sml: Testing Array2 operations.
...
val _ = tstStart "structure Array2"
...
in
  ...
  val test1b = tst0 "test1b" ((sub(a, 3, 0); "WRONG")
    handle Subscript => "OK" | _ => "WRONG");
  val test1c = tst0 "test1c" ((sub(a, 0, 4); "WRONG")
    handle Subscript => "OK" | _ => "WRONG");
  val test1d = tst0 "test1d" ((sub(a, ~1, 0); "WRONG")
    handle Subscript => "OK" | _ => "WRONG");
  val test1e = tst0 "test1e" ((sub(a, 0, ~1); "WRONG")
    handle Subscript => "OK" | _ => "WRONG");
  val test1f = tst0 "test1f" ((sub(a, ~1, ~1); "WRONG")
    handle Subscript => "OK" | _ => "WRONG");
  ...
  val test4c = tst0 "test4c" ((row(a, 4); "WRONG")
			      handle Subscript => "OK" | _ => "WRONG");
  val test4d = tst0 "test4d" ((row(a, ~1); "WRONG")
			      handle Subscript => "OK" | _ => "WRONG");
  ...
  val test5c = tst0 "test5c" ((column(a,  4); "WRONG")
			      handle Subscript => "OK" | _ => "WRONG");
  val test5d = tst0 "test5d" ((column(a, ~1); "WRONG")
			      handle Subscript => "OK" | _ => "WRONG");
 ...
 val test6h = tst0 "test6h"
  ...
  fun failcopy { row, col, nrows, ncols } dst_row dst_col = 
    (copy {src={ base=a1, row=row, col=col, nrows=nrows, ncols=ncols}, 
	   dst=a1, dst_col=dst_col, dst_row=dst_row }; "WRONG")
    handle Subscript => "OK" | _ => "WRONG"
  ...
  fun faili { row, col, nrows, ncols } = 
    ...
    in
      (appi RowMajor ignore reg; "WRONG") 
      handle Subscript => 
	(appi ColMajor ignore reg; "WRONG") 
	handle Subscript => 
	  (modifyi RowMajor (fn (_, _, a) => a) reg; "WRONG")
	  handle Subscript => 
	    (modifyi ColMajor (fn (_, _, a) => a) reg; "WRONG") 
	    handle Subscript => 
	      (foldi RowMajor (fn _ => 1) 0 reg; "WRONG") 
	      handle Subscript => 
		(foldi ColMajor (fn _ => 1) 0 reg; "WRONG") 
		handle Subscript => "OK"
		     | _ => "WRONG"
    end
  ...
  val test12i = tst0 "test12i" ((fromList [[], [2]]; "WRONG")
				handle Size => "OK" | _ => "WRONG")
  val test12j = tst0 "test12j" ((fromList [[2], []]; "WRONG")
				handle Size => "OK" | _ => "WRONG")
  val test12k = tst0 "test12k" ((fromList [[1, 2, 3], [4, 5, 6], [7, 8]]; "WRONG")
				handle Size => "OK" | _ => "WRONG")
  val test12l = tst0 "test12l" ((fromList [[1, 2, 3], [7, 8], [4, 5, 6]]; "WRONG")
				handle Size => "OK" | _ => "WRONG")
  val _ = tstEnd()
end
