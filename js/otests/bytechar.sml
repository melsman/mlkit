(* File bytechar.sml: Testing Byte and Char operations.
 * Copyright (c) 1994-2014, Peter Sestoft, Martin Elsman.
 * MIT License.
 *)

open Utest
val _ = tstStart "structures Byte and Char"
...
val test5 = tst0 "test5" ((Char.chr ~1; "WRONG") handle Chr => "OK" | _ => "WRONG")

val test6 = tst0 "test6" ((Char.chr (Char.maxOrd+1); "WRONG") 
...
val test9 = tst "test9" ("" = Byte.unpackString(Word8ArraySlice.full (Word8Array.fromList [])));

local 
    val arr = Word8Array.tabulate(10, fn i => Word8.fromInt(i+65))
in
val test10a = tst "test10a" ("" = Byte.unpackString(Word8ArraySlice.slice (arr, 0, SOME 0)));
val test10b = tst "test10b" ("" = Byte.unpackString(Word8ArraySlice.slice (arr, 10, SOME 0)) 
                             andalso "" = Byte.unpackString(Word8ArraySlice.slice (arr, 10, NONE)));
val test10c = tst "test10c" ("BCDE" = Byte.unpackString(Word8ArraySlice.slice (arr, 1, SOME 4)));
val test10d = tst0 "test10d" ((Byte.unpackString(Word8ArraySlice.slice (arr, ~1, SOME 0)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
val test10e = tst0 "test10e" ((Byte.unpackString(Word8ArraySlice.slice (arr, 11, SOME 0)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
val test10f = tst0 "test10f" ((Byte.unpackString(Word8ArraySlice.slice (arr, 0, SOME ~1)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
val test10g = tst0 "test10g" ((Byte.unpackString(Word8ArraySlice.slice (arr, 0, SOME 11)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
val test10h = tst0 "test10h" ((Byte.unpackString(Word8ArraySlice.slice (arr, 10, SOME 1)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
val test10i = tst0 "test10i" ((Byte.unpackString(Word8ArraySlice.slice (arr, ~1, NONE)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
val test10j = tst0 "test10j" ((Byte.unpackString(Word8ArraySlice.slice (arr, 11, NONE)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
end
...
in
val test11a = tst "test11a" ("" = Byte.unpackStringVec(Word8VectorSlice.slice(vec, 0, SOME 0)));
val test11b = tst "test11b" ("" = Byte.unpackStringVec(Word8VectorSlice.slice(vec, 10, SOME 0))
                             andalso "" = Byte.unpackStringVec(Word8VectorSlice.slice(vec, 10, NONE)));
val test11c = tst "test11c" ("BCDE" = Byte.unpackStringVec(Word8VectorSlice.slice(vec, 1, SOME 4)));
val test11d = tst0 "test11d" ((Byte.unpackStringVec(Word8VectorSlice.slice(vec, ~1, SOME 0)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
val test11e = tst0 "test11e" ((Byte.unpackStringVec(Word8VectorSlice.slice(vec, 11, SOME 0)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
val test11f = tst0 "test11f" ((Byte.unpackStringVec(Word8VectorSlice.slice(vec, 0, SOME ~1)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
val test11g = tst0 "test11g" ((Byte.unpackStringVec(Word8VectorSlice.slice(vec, 0, SOME 11)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
val test11h = tst0 "test11h" ((Byte.unpackStringVec(Word8VectorSlice.slice(vec, 10, SOME 1)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
val test11i = tst0 "test11i" ((Byte.unpackStringVec(Word8VectorSlice.slice(vec, ~1, NONE)); "WRONG")
                            handle Subscript => "OK" | _ => "WRONG")
val test11j = tst0 "test11j" ((Byte.unpackStringVec(Word8VectorSlice.slice(vec, 11, NONE)); "WRONG")
                              handle Subscript => "OK" | _ => "WRONG")
end
...
val test33 = 
    tst' "test33" (fn _ => 
           ...
           ; map toLower (explode graphchars)
           ; true))

val test34a =
    tst' "test34a" (fn _ => 
           map pred (List.drop(allchars, 1)) = List.take(allchars, 255));
val test34b = tst0 "test34b" ((pred minChar; "WRONG")
                              handle Chr => "OK" | _ => "WRONG")
val test35a =
    tst' "test35a" (fn _ => 
           map succ (List.take(allchars, 255)) = List.drop(allchars, 1));
val test35b = tst0 "test35b" ((succ maxChar; "WRONG")
                              handle Chr => "OK" | _ => "WRONG")
end
