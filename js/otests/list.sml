(* File list.sml: Testing List operations.
 * Copyright (c) 1994-2014, Peter Sestoft, Martin Elsman.
 * MIT License.
 *)

open Utest
val _ = tstStart "structure List"
...
val test3 = tst0 "test3" ((hd []; "WRONG") handle Empty => "OK" | _ => "WRONG")
val test4 = tst0 "test4" ((tl []; "WRONG") handle Empty => "OK" | _ => "WRONG")
val test5 = tst0 "test5" ((last []; "WRONG") handle Empty => "OK" | _ => "WRONG")

val test6 = tst "test6" (1 = nth(v123,0) andalso 3 = nth(v123,2))

val test7 = tst0 "test7" ((nth(v123,~1); "WRONG") handle Subscript => "OK" | _ => "WRONG")
val test8 = tst0 "test8" ((nth(v123,3); "WRONG") handle Subscript => "OK" | _ => "WRONG")
...
val test21 = (reset (); filter (fn i => (incrv i; true)) v16; checkv())

val test22 = tst "test22" (([2,4,6], [1,3,5]) = partition even v16)

val test23 = (reset (); partition (fn i => (incrv i; true)) v16; checkv())
...
val test32 = (reset(); tabulate (6, fn i => (incrv (i+1); 127)); checkv())

val test33 = tst "test33" ([] = tabulate (0, fn i => 1 div i))

val test34 = tst0 "test36b" ((tabulate(~1, fn _ => raise Div); "WRONG") 
                             handle Size => "OK" | _ => "WRONG")

val test35a = tst "test35a" (drop([], 0) = []
                   andalso drop(v123, 0) = v123
                   andalso drop(v123, 3) = [])
val test35b = tst0 "test36b" ((drop(v123, ~1); "WRONG") 
              handle Subscript => "OK" | _ => "WRONG")
val test35c = tst0 "test35c" ((drop(v123, 4); "WRONG") 
              handle Subscript => "OK" | _ => "WRONG")

val test36a = tst "test36a" (take([], 0) = [] 
                   andalso take(v123, 3) = v123
                   andalso take(v123, 0) = [])
val test36b = tst0 "test36b" ((take(v123, ~1); "WRONG") 
              handle Subscript => "OK" | _ => "WRONG")
val test36c = tst0 "test36c" ((take(v123, 4); "WRONG") 
              handle Subscript => "OK" | _ => "WRONG")
