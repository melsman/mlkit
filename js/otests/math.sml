(* File math.sml: Testing Math operations.
 * Copyright (c) 1994-2014, Peter Sestoft, Martin Elsman.
 * MIT License.
 *)

open Utest
val _ = tstStart "structure Math"
...
local
  ...
  fun check1 (opr, a, r) = check(opr a === r)
  fun check2 (opr, a1, a2, r) = check(opr(a1, a2) === r)
  ...
  val test1c = tst "test1c" (Real.isNan(sqrt ~1.0))
  ...
  val test5d = tst "test5d" (Real.isNan(asin 1.1))
  val test5e = tst "test5e" (Real.isNan(asin ~1.1))

  val test6a = tst1 "test6a" (acos, 1.0, 0.0);
  val test6b = tst1 "test6b" (acos, 0.0, pi/2.0);
  val test6c = tst1 "test6c" (acos, ~1.0, pi);
  val test6d = tst "test6d" (Real.isNan(acos 1.1))
  val test6e = tst "test6e" (Real.isNan(acos ~1.1))
  ...
  val test10d = tst "test10d" (Real.==(ln 0.0,Real.negInf))
  val test10e = tst "test10e" (Real.isNan(ln ~1.0))
  val test10f = tst "test10f" (Real.==(ln Real.posInf, Real.posInf))
  val test10g = tst "test10g" (Real.==(Real.posInf, Real.posInf))
  ...
  *)
  val test12l = tst "test12l" (Real.isNan(pow(~1.0, 1.1)))
  val test12m = tst "test12m" (Real.isNan(pow(~1.0, 0.5)))
  val test12n = tst "test12n" (Real.==(pow(3.0, 1000000.0),Real.posInf))   (* not in basis lib spec.*)
