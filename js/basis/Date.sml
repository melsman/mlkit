...
structure Date :> DATE =
  struct
    ...
    fun tmozToDate {tm_hour, tm_isdst, tm_mday, tm_min, tm_mon, tm_sec,
        ...
        DATE {year = tm_year, month = tomonth tm_mon, 
...
*)
    ...
    local
      ...
      val tomonth   = fn 0 => "Jan" | 1 => "Feb" | 2 => "Mar" | 3 => "Apr"
                       ...
                       | 8 => "Sep" | 9 => "Oct" | 10 => "Nov" | 11 => "Dec"
                       | _ => raise Fail "Internal error: Date.tomonth";
      val toweekdayL= fn 0 => "Sunday" | 1 => "Monday" | 2 => "Tuesday" | 3 => "Wednesday"
                       | 4 => "Thursday" | 5 => "Friday" | 6 => "Saturday"
                       | _ => raise Fail "Internal error: Date.toweekdayL";
      val tomonthL  = fn 0 => "January" | 1 => "Febuary" | 2 => "March" | 3 => "April"
                       | 4 => "May" | 5 => "June" | 6 => "July" | 7 => "August"
                       | 8 => "September" | 9 => "October" | 10 => "November" | 11 => "December"
                       | _ => raise Fail "Internal error: Date.tomonthL";
      ...
      fun to12 0 = 12
        | to12 n = if n < 13 then n
                   else n - 12

      fun top n = if n < 12 then "am" else "pm"

      fun strftime (s : string, t : tmoz) : string = 
          let fun loop nil = nil
                | loop (#"%"::nil) = nil
                | loop (#"%"::c::cs) : string list =
                  (case c of
                     #"%" => "%" :: loop cs
                   | #"a" => toweekday(#tm_wday t) :: loop cs
                   | #"A" => toweekdayL(#tm_wday t) :: loop cs
                   | #"b" => tomonth(#tm_mon t) :: loop cs
                   | #"B" => tomonthL(#tm_mon t) :: loop cs
                   | #"c" => loop(explode "%b %d %H:%M:%S %Y" @ cs)
                   | #"d" => zeroExt 2 (Int.toString(#tm_mday t)) :: loop cs
                   | #"H" => zeroExt 2 (Int.toString(#tm_hour t)) :: loop cs
                   | #"I" => zeroExt 2 (Int.toString(to12(#tm_hour t))) :: loop cs
                   | #"j" => zeroExt 3 (Int.toString(#tm_yday t)) :: loop cs
                   | #"m" => zeroExt 2 (Int.toString((#tm_mon t)+1)) :: loop cs
                   | #"M" => zeroExt 2 (Int.toString(#tm_min t)) :: loop cs
                   | #"p" => top(#tm_hour t) :: loop cs
                   | #"S" => zeroExt 2 (Int.toString(#tm_sec t)) :: loop cs
                   | #"U" => zeroExt 2 (Int.toString((#tm_yday t + 7 - #tm_wday t)div 7 + 1)) :: loop cs
                   | #"w" => zeroExt 2 (Int.toString(#tm_wday t)) :: loop cs
                   | #"W" => zeroExt 2 (Int.toString((#tm_yday t + 6 - #tm_wday t)div 7 + 1)) :: loop cs
                   | #"x" => loop(explode "%b %d %Y" @ cs)
                   | #"y" => zeroExt 2 (Int.toString(#tm_year t mod 100)) :: loop cs
                   | #"Y" => Int.toString(#tm_year t) :: loop cs
                   | #"Z" => loop cs
                   | c => Char.toString c :: loop cs)
                | loop (c::cs) = Char.toString c :: loop cs
          in concat(loop(explode s))
          end
    in
      fun fmt fmtstr date =
          (strftime (fmtstr, dateToTmoz date)) 
          handle Fail _ => raise Date
    end
