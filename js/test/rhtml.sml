...
*)
  ...
  datatype attr = A of string * string b  
                ...
                | E of Js.eventType * (Js.elem -> bool)
  ...
  fun elema t (a,e) =
      let val pR = ref NONE   (* parent ref *)
          val (aa,ss,vv) = 
              List.foldl (fn (A(x,y),(aa,ss,vv)) => (pair(const x,y)::aa,ss,vv)
                           | (S(x,y),(aa,ss,vv)) => (aa,pair(const x,y)::ss,vv)
                           | (E(x,y),(aa,ss,vv)) => (aa,ss,(x,y)::vv)) 
                         (nil,nil,nil) a
      in arr (fn (es,aa,ss) => 
                 let val p =
                         case !pR of
                         ...
                         | NONE => 
                           ...
                           in List.app (Js.appendChild p) es
                            (* install event handlers - they are static, so 
                             * we only need to install them once... *)
                            ; app (fn (t,f) => Js.installEventHandler p t (fn() => f p)) vv                              
                            ; pR := SOME p
                            ; p
                           end
