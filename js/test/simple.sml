...
fun printarray2 (A as (M:real array2,((l1,u1),(l2,u2)))) =
    for {from=l1,step=1,to=u1} (fn i =>
	 ...
	 println ( (* makestring (sub2(A,(i,u2))) ^ *) "]")))
...
fun make_cc(alpha_prime, theta_hat) =
    let fun interior_cc zone =  
	    ...
	    handle Sqrt => (println ("[real]" (*Real.makestring (sub2(theta_hat, zone))*));
			    println ("zone =(" (* ^ makestring (#1 zone) *) ^ "," ^ 
				   (* makestring (#2 zone) ^ *) ")");
...
fun make_sigma(deltat, rho_prime, alpha_prime) =
	...
	fun ohandle zone =
	    (print ( (* makestring (sub2(rho_prime, zone)) ^ *)" ");
	     ...
	     println ( (* makestring deltat ^ *) "");
	     raise Overflow)
		    
    in  if !trace 
        then println ("make_sigma:deltat = " (* ^ makestring deltat *) ^ "")
	else ();
...
fun compute_heat_conduction(theta_hat, deltat, x', alpha', rho') =
	...
	val _ = if !trace then println "done make_sigma" else ()

	val cc 		= make_cc(alpha',  theta_hat)
	val _ = if !trace then println "done make_cc" else ()

	val Gamma_k  	= make_gamma(  x', cc, north, east)
	val _ = if !trace then println "done make_gamma" else ()

	val (a_k,b_k)  	= make_ab(theta_hat, sigma, Gamma_k, north)
	val _ = if !trace then println "done make_ab" else ()

	val theta_k  	= make_theta(a_k,b_k,south,for_north_ward_interior_zones)
	val _ = if !trace then println "done make_theta" else ()
...
fun compute_next_state state =
	...
	val _ = if !trace then println "done make_velocity" else ()
	...
	val x'  = make_position(x,deltat,v') 
	          handle _ => ( (* old: handle Overflow => *)
				     ...
				     printarray2 (#2 v');
				     raise Overflow)
	val _ = if !trace then println "done make_position" else ()

	val (alpha',rho',s')  = make_area_density_volume (rho,  s , x')
	val _ = if !trace then println "done make_area_density_volume"
		else ()

	val (q',d)  = make_viscosity (p,  v',  x',  alpha',  rho')
	val _ = if !trace then println "done make_viscosity" else ()

	val theta_hat  = make_temperature (p, epsilon, rho, theta, rho', q')
	val _ = if !trace then println "done make_temperature" else ()

	val (theta',Gamma_k,Gamma_l) =
	    compute_heat_conduction (theta_hat, deltat, x', alpha', rho')
	val _ = if !trace then println "done compute_heat_conduction"
		else ()

	val p'  = make_pressure(rho', theta')
	val _ = if !trace then println "done make_pressure" else ()

	val epsilon'  = make_energy (rho', theta')
	val _ = if !trace then println "done make_energy" else ()

	val c'  = compute_energy_error (v', x', p', q', epsilon', theta', rho', 
					alpha', Gamma_k, Gamma_l,  deltat)
	val _ = if !trace then println "done compute_energy_error" 
		else ()

	val deltat'  = compute_time_step (d, theta_hat, theta')
	val _ = if !trace then println "done compute_time_step" else ()
    in
	(v',x',alpha',s',rho',p',q',  epsilon',theta',deltat',c')
    end
...
fun print_state ((v1,v2),(r,z),alpha,s,rho,p,q,epsilon,theta,deltat,c) = (
      println "Velocity matrices = ";
      printarray2 v1; println "";
      printarray2 v2;

      println "Position matrices = ";
      printarray2 r; println "";
      printarray2 z;
      ...
      println ("delatat = " (* ^ Real.makestring deltat *) ^ "");
      println ("c = " (* ^ Real.makestring c *) ^ ""))
    ...
    fun doit () = let
	  ...
	  val _ = println(int_to_string(c))
	  val _ = println(int_to_string(delta))
	  in
...
fun timeIt f =
    let
      fun reportTimer s rt =
          let val t = Timer.checkRealTimer rt
          in print ("<h4>" ^ s ^ " : " ^ Time.toString t ^ "</h4>\n")
          end
      val rt = Timer.startRealTimer()
      val trt = Timer.totalRealTimer()
    in f ()
     ; reportTimer "Compute time" rt
     ; reportTimer "Total time" trt
    end

val () = print "<h2>Simple</h2>\n"
